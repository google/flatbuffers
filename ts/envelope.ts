...
export function envelopeWrap(
  ...
  if (schemaHash.length !== 32) {
    throw new Error(
      `envelopeWrap: schemaHash must be 32 bytes, got ${schemaHash.length}`,
    );
  }
 ...
 */
export function envelopeUnwrap(
  data: Uint8Array,
 ...
 */
export function envelopePayload(data: Uint8Array): Uint8Array | null {
  const parsed = parseEnvelope(data);
 ...
 *          header is malformed.
 */
export function envelopeSchemaHash(data: Uint8Array): Uint8Array | null {
  const parsed = parseEnvelope(data);
...
function parseEnvelope(data: Uint8Array): ParsedEnvelope | null {
  if (data.length < ENVELOPE_HEADER_SIZE) {
    return null;
  }
  // magic check
