...
function readFbString(buf: Uint8Array, fieldPos: number): string {
  ...
  return new TextDecoder().decode(
    buf.subarray(strStart + 4, strStart + 4 + length),
  );
}
...
function resolveUnionDiscriminant(
...
function unpackFieldIntoRecord(
  ...
  switch (fieldType.baseType()) {
    ...
    case ReflectionBaseType.Byte:
      result[name] =
        buf.length > abs ? (buf[abs]! > 127 ? buf[abs]! - 256 : buf[abs]!) : 0;
      break;
  ...
  switch (elem) {
    ...
    case ReflectionBaseType.Obj: {
      ...
      if (idx >= 0 && idx < schema.objects().length) {
        ...
        for (let i = 0; i < count; i++) {
          const elemPos = dataStart + i * 4;
          const childPos = childObj.isStruct()
            ? elemPos
            : elemPos + readUint32(buf, elemPos);
    ...
    case ReflectionBaseType.ULong: {
      ...
      if (val != null)
        builder.prependUint64Slot(
          slot,
          BigInt(val as number | bigint | string),
        );
      break;
    }
    case ReflectionBaseType.Float: {
...
function packUTypeSlot(
  ...
  const unionFieldName = name.endsWith(typeSuffix)
    ? name.slice(0, -typeSuffix.length)
    : name;
...
class FbBuilder {
  ...
  private grow(neededSpace: number): void {
    ...
    newBytes.set(
      this.bytes.subarray(this.head),
      newSize - (this.capacity() - this.head),
    );
    this.buf = newBuf;
    this.view = new DataView(this.buf);
    this.bytes = newBytes;
  ...
  private align(size: number): void {
    ...
    const alignSize =
      (~(this.capacity() - this.head + this.space) + 1) & (size - 1);
  ...
  endObject(): number {
    ...
    for (let i = 0; i < numSlots; i++) {
      const voff =
        this.vtable[i] === 0 ? 0 : this.vtable[i]! - this.objectStart;
    ...
    const vtableHeadPos =
      vtableOffset === 0 ? tableHeadPos : this.capacity() - vtableOffset;
