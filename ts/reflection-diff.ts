...
function diffFloatField(
  ...
  if (absentA) {
    ...
    if (
      valB !== null &&
      valB !== defaultVal &&
      !(Number.isNaN(valB) && Number.isNaN(defaultVal))
    ) {
      deltas.push({ path, kind: "added", oldValue: null, newValue: valB });
    }
    return;
  ...
  if (absentB) {
    ...
    if (
      valA !== null &&
      valA !== defaultVal &&
      !(Number.isNaN(valA) && Number.isNaN(defaultVal))
    ) {
      deltas.push({ path, kind: "removed", oldValue: valA, newValue: null });
    }
    return;
...
function floatEqual(
  ...
  b: number | bigint | string | boolean,
): boolean {
...
function diffVectorField(
  ...
  const metaA = absentA
    ? { count: 0, dataStart: 0 }
    : getVecMeta(bufA, tablePosA);
  const metaB = absentB
    ? { count: 0, dataStart: 0 }
    : getVecMeta(bufB, tablePosB);
  ...
  if (elemType === ReflectionBaseType.Obj) {
    ...
    for (let i = 0; i < minLen; i += 1) {
      ...
      if (slotA + 4 <= bufA.byteLength && slotB + 4 <= bufB.byteLength) {
        ...
        if (
          childPosA >= 0 &&
          childPosA < bufA.byteLength &&
          childPosB >= 0 &&
          childPosB < bufB.byteLength
        ) {
          diffTable(
            bufA,
            childPosA,
