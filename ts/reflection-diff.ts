...
function resolveFieldPos(
  ...
  if (vtablePos < 0 || vtablePos + 2 > dataBuf.byteLength) return null;
  const vtableSize = dataBuf.getUint16(vtablePos, true);
  if (voffset >= vtableSize) return null;
 ...
 */
function isAbsent(dataBuf: DataView, tablePos: number, field: ReflectionField): boolean {
 ...
 * the buffer.
 */
function followTable(buf: DataView, fieldPos: number): number | null {
  if (fieldPos + 4 > buf.byteLength) return null;
  const uoffset = buf.getUint32(fieldPos, true);
  const tablePos = fieldPos + uoffset;
  if (tablePos < 0 || tablePos >= buf.byteLength) return null;
  return tablePos;
}
...
function readVectorMeta(
  buf: DataView,
  fieldPos: number,
): { count: number; dataStart: number } | null {
  if (fieldPos + 4 > buf.byteLength) return null;
  const uoffset = buf.getUint32(fieldPos, true);
  const vecBase = fieldPos + uoffset;
  if (vecBase < 0 || vecBase + 4 > buf.byteLength) return null;
...
function diffObjField(
  ...
  if (idx >= allObjects.length) return;
...
function diffStructInline(
  ...
  if (posA + byteSize > bufA.byteLength || posB + byteSize > bufB.byteLength) return;
  ...
  for (const field of obj.fields()) {
    ...
    switch (ft.baseType()) {
      case ReflectionBaseType.Bool: {
        if (absA >= bufA.byteLength || absB >= bufB.byteLength) break;
        const valA = bufA.getUint8(absA) !== 0;
        const valB = bufB.getUint8(absB) !== 0;
        if (valA !== valB) deltas.push({ path, kind: 'changed', oldValue: valA, newValue: valB });
        break;
      }
      case ReflectionBaseType.Byte: {
        if (absA >= bufA.byteLength || absB >= bufB.byteLength) break;
      ...
      case ReflectionBaseType.Short: {
        if (absA + 2 > bufA.byteLength || absB + 2 > bufB.byteLength) break;
      ...
      case ReflectionBaseType.Int: {
        if (absA + 4 > bufA.byteLength || absB + 4 > bufB.byteLength) break;
      ...
      case ReflectionBaseType.Long: {
        if (absA + 8 > bufA.byteLength || absB + 8 > bufB.byteLength) break;
...
const SCALAR_SIZE: Partial<Record<ReflectionBaseType, number>> = {
 ...
 */
function readVectorScalar(
  buf: DataView,
  pos: number,
  elemType: ReflectionBaseType,
  ...
  switch (elemType) {
    ...
    case ReflectionBaseType.String: {
      ...
      if (strBase + 4 > buf.byteLength) return null;
      const length = buf.getUint32(strBase, true);
      if (strBase + 4 + length > buf.byteLength) return null;
 ...
 */
function floatEqual(a: number | bigint | string | boolean, b: number | bigint | string | boolean): boolean {
...
function diffVectorField(
  ...
  const getVecMeta = (buf: DataView, tablePos: number): { count: number; dataStart: number } | null => {
    const fieldPos = resolveFieldPos(buf, tablePos, field.offset());
    if (fieldPos === null) return { count: 0, dataStart: 0 }; // absent = empty
  ...
  if (elemType === ReflectionBaseType.Obj) {
    ...
    for (let i = 0; i < minLen; i++) {
      ...
      if (slotA + 4 > bufA.byteLength || slotB + 4 > bufB.byteLength) continue;

      const childPosA = slotA + bufA.getUint32(slotA, true);
      const childPosB = slotB + bufB.getUint32(slotB, true);

      if (childPosA < 0 || childPosA >= bufA.byteLength) continue;
      if (childPosB < 0 || childPosB >= bufB.byteLength) continue;
    ...
    if (stride === 0) return; // unsupported element type
