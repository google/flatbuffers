 ...
 */
function readVectorTableElement(
  buf: DataView,
  dataStart: number,
  index: number,
): number {
...
export function getFieldInt(
  ...
  const fieldType = field.type();
  const base =
    fieldType !== null ? fieldType.baseType() : ReflectionBaseType.Int;
  switch (base) {
...
export function getFieldFloat(
  ...
  const fieldType = field.type();
  const base =
    fieldType !== null ? fieldType.baseType() : ReflectionBaseType.Float;
