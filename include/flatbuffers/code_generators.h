...
class BaseGenerator {
 public:
  ...
  static const std::string NamespaceName(const Parser &parser,
                                         const char *namespace_separator) {
    std::string namespace_name;
    auto &namespaces = parser.namespaces_.back()->components;
    for (auto it = namespaces.begin(); it != namespaces.end(); ++it) {
      if (namespace_name.length()) namespace_name += namespace_separator;
      namespace_name += *it;
    }
 ...
 protected:
  BaseGenerator(const Parser &parser, const std::string &path,
                const std::string &file_name, const char *namespace_separator)
      : parser_(parser),
        path_(path),
        file_name_(file_name),
        namespace_name_(
            BaseGenerator::NamespaceName(parser, namespace_separator)),
