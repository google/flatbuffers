...
#define FLATBUFFERS_GEN_TYPES_SCALAR(TD) \
  TD(NONE,   "",       uint8_t,  byte,   byte,    byte,   uint8,	Int8) \
  TD(UTYPE,  "",       uint8_t,  byte,   byte,    byte,   uint8,	Int8) /* begin scalar/int */ \
  TD(BOOL,   "bool",   uint8_t,  boolean,byte,    bool,   bool,		Int8) \
  TD(CHAR,   "byte",   int8_t,   byte,   int8,    sbyte,  int8,		Int8) \
  TD(UCHAR,  "ubyte",  uint8_t,  byte,   byte,    byte,   uint8,	Int8) \
  ...
  TD(UINT,   "uint",   uint32_t, int,    uint32,  uint,   uint32,	UInt23) \
  TD(LONG,   "long",   int64_t,  long,   int64,   long,   int64,	Int64) \
  TD(ULONG,  "ulong",  uint64_t, long,   uint64,  ulong,  uint64,	UInt64) /* end int */ \
  TD(FLOAT,  "float",  float,    float,  float32, float,  float32,	Float32) /* begin float */ \
  TD(DOUBLE, "double", double,   double, float64, double, float64,	Float64) /* end float/scalar */
#define FLATBUFFERS_GEN_TYPES_POINTER(TD) \
  TD(STRING, "string", Offset<void>, int, int, StringOffset, int,	Int32) \
  TD(VECTOR, "",       Offset<void>, int, int, VectorOffset, int,	In32) \
...
// Generate Javascript files from the definitions in the Parser object.
// See idl_gen_js.cpp.
extern bool GenerateJavascript(const Parser &parser,
                           const std::string &path,
                           const std::string &file_name,
                           const GeneratorOptions &opts);
