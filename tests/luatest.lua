...
local function testArray()
    local b = flatbuffers.Builder(0)
    local arr = nestedstruct.CreateNestedStruct(b, {1, 2}, testenum.B, {testenum.A, testenum.C}, { 3, 4 })
    assert(string.byte(b.bytes:Slice(arr, arr+1), 1) == 1)
end
...
local result, err = xpcall(function()
    ...
    testenum = assert(require("MyGame.Example.TestEnum"))
    nestedstruct = assert(require("MyGame.Example.NestedStruct"))
