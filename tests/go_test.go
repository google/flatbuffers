...
func TestAll(t *testing.T) {
	...
	// Check that --go-nsimport-prefix works
	CheckNSImportPrefix(t.Fatalf)
...
// CheckNSImportPrefix tests that nsimport-prefix path has been correctly
// prepended to generated namespace import path using the import in
// ./MyGame/Example/Monster.go as an example
func CheckNSImportPrefix(fail func(string, ...interface{})) {
	nsimport_testfile := "./MyGame/Example/Monster.go"
	f, err := parser.ParseFile(token.NewFileSet(), nsimport_testfile, nil, parser.ImportsOnly)
	if err != nil {
		fail("Failed to parse NSImportPrefix testfile's imports:\n", err)
	}

	// Collect the import identifier-path pairs from the file's AST into a map.
	imports := map[string]string{}
	for _, s := range f.Imports {
		imports[s.Name.Name] = s.Path.Value
	}

	const want = "\"fake.flatbuffers.moduleroot/tests/MyGame\""
	if got := imports["MyGame"]; got != want {
		fail(FailString("NSImportPrefix", want, got))
	}
}
