...
function applyPrototypeAliases(classes) {
  var visited = new Set();
  classes.forEach(function(ctor) {
    if (typeof ctor !== 'function' || ctor === null || visited.has(ctor)) {
      return;
    }
    visited.add(ctor);
    ...
    Object.getOwnPropertyNames(proto).forEach(function(key) {
      ...
      var value = proto[key];
      if (typeof value !== 'function' || key.indexOf('_') === -1) {
        return;
      }
      var camel = key.replace(/_+([a-zA-Z0-9])/g, function(_, ch) {
        return ch.toUpperCase();
      });
      if (camel !== key && !Object.prototype.hasOwnProperty.call(proto, camel)) {
...
applyPrototypeAliases([
  MyGame && MyGame.Example && MyGame.Example.Monster,
  MyGame && MyGame.Example && MyGame.Example.MonsterT,
  MyGame && MyGame.Example && MyGame.Example.Test,
  MyGame && MyGame.Example && MyGame.Example.TestT,
  MyGame && MyGame.Example && MyGame.Example.Vec3,
  MyGame && MyGame.Example && MyGame.Example.Stat,
]);
