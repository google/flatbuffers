...
// Test that clone() on Object API types produces correct array copies
// for both scalar vectors (string[], number[]) and struct vectors (TestT[]).
// Regression: clone() previously generated null-checks and null returns
// for non-nullable vector fields, producing (T|null)[] instead of T[].
function testObjApiCloneVectors() {
  var fbb = new flatbuffers.Builder(1);
  createMonster(fbb);
  var monster = Monster.getRootAsMonster(fbb.dataBuffer()).unpack();

  var cloned = monster.clone();

  // Scalar vector (string[]): spread copy, no null
  assert.ok(Array.isArray(cloned.testarrayofstring));
  assert.strictEqual(cloned.testarrayofstring.length, 2);
  assert.strictEqual(cloned.testarrayofstring[0], 'test1');
  assert.strictEqual(cloned.testarrayofstring[1], 'test2');
  // Must be a new array, not the same reference
  assert.notStrictEqual(cloned.testarrayofstring, monster.testarrayofstring);

  // Scalar vector (number[]): spread copy, no null
  assert.ok(Array.isArray(cloned.inventory));
  assert.strictEqual(cloned.inventory.length, 5);
  assert.notStrictEqual(cloned.inventory, monster.inventory);

  // Struct vector (TestT[]): element-wise clone, no null entries
  assert.ok(Array.isArray(cloned.test4));
  assert.strictEqual(cloned.test4.length, 2);
  for (var i = 0; i < cloned.test4.length; i++) {
    assert.ok(cloned.test4[i] instanceof TestT, 'test4[' + i + '] should be TestT, not null');
    assert.notStrictEqual(cloned.test4[i], monster.test4[i], 'test4[' + i + '] should be a new instance');
  }
  assert.strictEqual(cloned.test4[0].a + cloned.test4[0].b + cloned.test4[1].a + cloned.test4[1].b, 100);

  // Verify empty vectors clone to empty arrays, not null
  var empty = new MonsterT();
  var emptyClone = empty.clone();
  assert.ok(Array.isArray(emptyClone.testarrayofstring));
  assert.strictEqual(emptyClone.testarrayofstring.length, 0);
  assert.ok(Array.isArray(emptyClone.test4));
  assert.strictEqual(emptyClone.test4.length, 0);
  assert.ok(Array.isArray(emptyClone.inventory));
  assert.strictEqual(emptyClone.inventory.length, 0);
}

// Test that equals() on Object API types compares vectors directly
// without null-guard wrappers on non-nullable array fields.
// Also verifies Object.is() is used for float fields (NaN === NaN).
function testObjApiEqualsVectors() {
  var fbb = new flatbuffers.Builder(1);
  createMonster(fbb);
  var monster = Monster.getRootAsMonster(fbb.dataBuffer()).unpack();
  var cloned = monster.clone();

  // Cloned object should be equal (including NaN default fields)
  assert.ok(monster.equals(cloned));
  assert.ok(cloned.equals(monster));

  // Mutate a string vector element — should no longer be equal
  cloned.testarrayofstring[0] = 'changed';
  assert.ok(!monster.equals(cloned));

  // Restore and mutate a struct vector element
  cloned = monster.clone();
  cloned.test4[0] = new TestT(99, 99);
  assert.ok(!monster.equals(cloned));

  // Different vector lengths should not be equal
  cloned = monster.clone();
  cloned.inventory.push(255);
  assert.ok(!monster.equals(cloned));

  // Empty monsters should be equal (NaN defaults handled by Object.is)
  var a = new MonsterT();
  var b = new MonsterT();
