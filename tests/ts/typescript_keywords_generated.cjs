...
function keyValueVerify(verifier, tablePos) {
  let result = true;
  result = result && verifier.verifyTableStart(tablePos);
  result = result && verifier.verifyString(tablePos, 4, true);
  result = result && verifier.verifyString(tablePos, 6, false);
...
function typeVerify(verifier, tablePos) {
  let result = true;
  result = result && verifier.verifyTableStart(tablePos);
  result = result && verifier.verifyField(tablePos, 4, 1, 1, false);
  result = result && verifier.verifyField(tablePos, 6, 1, 1, false);
  result = result && verifier.verifyField(tablePos, 8, 4, 4, false);
  result = result && verifier.verifyField(tablePos, 10, 2, 2, false);
  result = result && verifier.verifyField(tablePos, 12, 4, 4, false);
  result = result && verifier.verifyField(tablePos, 14, 4, 4, false);
  result = result && verifier.verifyTableEnd(tablePos);
  return result;
}
...
function enumValVerify(verifier, tablePos) {
  let result = true;
  result = result && verifier.verifyTableStart(tablePos);
  result = result && verifier.verifyString(tablePos, 4, true);
  result = result && verifier.verifyField(tablePos, 6, 8, 8, false);
  result = result && verifier.verifyTable(tablePos, 10, typeVerify, false);
  result = result && verifier.verifyVectorOfStrings(tablePos, 12, false);
  result = result && verifier.verifyVectorOfTables(tablePos, 14, keyValueVerify, false);
  result = result && verifier.verifyTableEnd(tablePos);
  return result;
}
...
function enum_Verify(verifier, tablePos) {
  let result = true;
  result = result && verifier.verifyTableStart(tablePos);
  result = result && verifier.verifyString(tablePos, 4, true);
  result = result && verifier.verifyVectorOfTables(tablePos, 6, enumValVerify, true);
  result = result && verifier.verifyField(tablePos, 8, 1, 1, false);
  result = result && verifier.verifyTable(tablePos, 10, typeVerify, true);
  result = result && verifier.verifyVectorOfTables(tablePos, 12, keyValueVerify, false);
  result = result && verifier.verifyVectorOfStrings(tablePos, 14, false);
  result = result && verifier.verifyString(tablePos, 16, false);
  result = result && verifier.verifyTableEnd(tablePos);
  return result;
}
...
function fieldVerify(verifier, tablePos) {
  let result = true;
  result = result && verifier.verifyTableStart(tablePos);
  result = result && verifier.verifyString(tablePos, 4, true);
  result = result && verifier.verifyTable(tablePos, 6, typeVerify, true);
  result = result && verifier.verifyField(tablePos, 8, 2, 2, false);
  result = result && verifier.verifyField(tablePos, 10, 2, 2, false);
  result = result && verifier.verifyField(tablePos, 12, 8, 8, false);
  result = result && verifier.verifyField(tablePos, 14, 8, 8, false);
  result = result && verifier.verifyField(tablePos, 16, 1, 1, false);
  result = result && verifier.verifyField(tablePos, 18, 1, 1, false);
  result = result && verifier.verifyField(tablePos, 20, 1, 1, false);
  result = result && verifier.verifyVectorOfTables(tablePos, 22, keyValueVerify, false);
  result = result && verifier.verifyVectorOfStrings(tablePos, 24, false);
  result = result && verifier.verifyField(tablePos, 26, 1, 1, false);
  result = result && verifier.verifyField(tablePos, 28, 2, 2, false);
  result = result && verifier.verifyField(tablePos, 30, 1, 1, false);
  result = result && verifier.verifyTableEnd(tablePos);
  return result;
}
...
function object_Verify(verifier, tablePos) {
  let result = true;
  result = result && verifier.verifyTableStart(tablePos);
  result = result && verifier.verifyString(tablePos, 4, true);
  result = result && verifier.verifyVectorOfTables(tablePos, 6, fieldVerify, true);
  result = result && verifier.verifyField(tablePos, 8, 1, 1, false);
  result = result && verifier.verifyField(tablePos, 10, 4, 4, false);
  result = result && verifier.verifyField(tablePos, 12, 4, 4, false);
  result = result && verifier.verifyVectorOfTables(tablePos, 14, keyValueVerify, false);
  result = result && verifier.verifyVectorOfStrings(tablePos, 16, false);
  result = result && verifier.verifyString(tablePos, 18, false);
  result = result && verifier.verifyTableEnd(tablePos);
  return result;
}
...
function rpccallVerify(verifier, tablePos) {
  let result = true;
  result = result && verifier.verifyTableStart(tablePos);
  result = result && verifier.verifyString(tablePos, 4, true);
  result = result && verifier.verifyTable(tablePos, 6, object_Verify, true);
  result = result && verifier.verifyTable(tablePos, 8, object_Verify, true);
  result = result && verifier.verifyVectorOfTables(tablePos, 10, keyValueVerify, false);
  result = result && verifier.verifyVectorOfStrings(tablePos, 12, false);
  result = result && verifier.verifyTableEnd(tablePos);
  return result;
}
...
function schemaFileVerify(verifier, tablePos) {
  let result = true;
  result = result && verifier.verifyTableStart(tablePos);
  result = result && verifier.verifyString(tablePos, 4, true);
  result = result && verifier.verifyVectorOfStrings(tablePos, 6, false);
  result = result && verifier.verifyTableEnd(tablePos);
  return result;
}
...
function serviceVerify(verifier, tablePos) {
  let result = true;
  result = result && verifier.verifyTableStart(tablePos);
  result = result && verifier.verifyString(tablePos, 4, true);
  result = result && verifier.verifyVectorOfTables(tablePos, 6, rpccallVerify, false);
  result = result && verifier.verifyVectorOfTables(tablePos, 8, keyValueVerify, false);
  result = result && verifier.verifyVectorOfStrings(tablePos, 10, false);
  result = result && verifier.verifyString(tablePos, 12, false);
  result = result && verifier.verifyTableEnd(tablePos);
  return result;
}
...
var Schema = class _Schema {
  ...
  static verifySchema(bb, sizePrefix) {
    let verifier = flatbuffers10.newVerifier(bb);
    if (sizePrefix == null)
      sizePrefix = true;
...
function schemaVerify(verifier, tablePos) {
  let result = true;
  result = result && verifier.verifyTableStart(tablePos);
  result = result && verifier.verifyVectorOfTables(tablePos, 4, object_Verify, true);
  result = result && verifier.verifyVectorOfTables(tablePos, 6, enum_Verify, true);
  result = result && verifier.verifyString(tablePos, 8, false);
  result = result && verifier.verifyString(tablePos, 10, false);
  result = result && verifier.verifyTable(tablePos, 12, object_Verify, false);
  result = result && verifier.verifyVectorOfTables(tablePos, 14, serviceVerify, false);
  result = result && verifier.verifyField(tablePos, 16, 8, 8, false);
  result = result && verifier.verifyVectorOfTables(tablePos, 18, schemaFileVerify, false);
  result = result && verifier.verifyTableEnd(tablePos);
  return result;
}
