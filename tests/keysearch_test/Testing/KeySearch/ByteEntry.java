...
public final class ByteEntry extends Table {
  ...
  public static int lookupByKey( int bb_pos, int fieldDataOffset, byte key, byte defaultKeyValue , ByteBuffer bb) {
    ...
    while (span != 0) {
      ...
      byte val = keyValueOffset != 0 ? bb.get(tableOffset + keyValueOffset) : defaultKeyValue;
      if (key < val) {
        span = middle;
      } else if (key > val) {
        middle++;
