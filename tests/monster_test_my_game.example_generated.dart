...
class StringKey {
  StringKey._(this._bc, this._bcOffset);
  factory StringKey(List<int> bytes) {
    fb.BufferContext rootRef = new fb.BufferContext.fromBytes(bytes);
    return reader.read(rootRef, 0);
  }

  static const fb.Reader<StringKey> reader = const _StringKeyReader();

  final fb.BufferContext _bc;
  final int _bcOffset;

  String get k => const fb.StringReader().vTableGet(_bc, _bcOffset, 4, null);

  @override
  String toString() {
    return 'StringKey{k: $k}';
  }
}

class _StringKeyReader extends fb.TableReader<StringKey> {
  const _StringKeyReader();

  @override
  StringKey createObject(fb.BufferContext bc, int offset) => 
    new StringKey._(bc, offset);
}

class StringKeyBuilder {
  StringKeyBuilder(this.fbBuilder) {
    assert(fbBuilder != null);
  }
...
class StringKeyObjectBuilder extends fb.ObjectBuilder {
  final String _k;

  StringKeyObjectBuilder({
    String k,
  })
      : _k = k;
  ...
  int finish(
    fb.Builder fbBuilder) {
    assert(fbBuilder != null);
    final int kOffset = fbBuilder.writeString(_k);

    fbBuilder.startTable();
    if (kOffset != null) {
      fbBuilder.addOffset(0, kOffset);
    }
    return fbBuilder.endTable();
  }
