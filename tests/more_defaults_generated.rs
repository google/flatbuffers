...
impl<'a> MoreDefaults<'a> {
    ...
    pub fn create<'bldr: 'args, 'args: 'mut_bldr, 'mut_bldr>(
      ...
      builder.add_some_string(args.some_string);
      builder.add_empty_string(args.empty_string);
      builder.add_floats(args.floats);
      builder.add_ints(args.ints);
      builder.finish()
    }
  ...
  #[inline]
  pub fn ints(&self) -> Option<flatbuffers::Vector<'a, i32>> {
    self._tab.get::<flatbuffers::ForwardsUOffset<flatbuffers::Vector<'a, i32>>>(MoreDefaults::VT_INTS, Some([])).unwrap()
  }
  #[inline]
  pub fn floats(&self) -> Option<flatbuffers::Vector<'a, f32>> {
    self._tab.get::<flatbuffers::ForwardsUOffset<flatbuffers::Vector<'a, f32>>>(MoreDefaults::VT_FLOATS, Some([])).unwrap()
  }
  #[inline]
  pub fn empty_string(&self) -> Option<&'a str> {
    self._tab.get::<flatbuffers::ForwardsUOffset<&str>>(MoreDefaults::VT_EMPTY_STRING, Some()).unwrap()
  }
  #[inline]
  pub fn some_string(&self) -> Option<&'a str> {
...
pub struct MoreDefaultsArgs<'a> {
    pub ints: flatbuffers::WIPOffset<flatbuffers::Vector<'a, i32>>,
    pub floats: flatbuffers::WIPOffset<flatbuffers::Vector<'a, f32>>,
    pub empty_string: flatbuffers::WIPOffset<&'a str>,
    pub some_string: flatbuffers::WIPOffset<&'a str>,
}
...
impl MoreDefaultsT {
  pub fn pack<'b>(
    ...
    let ints = self.ints.as_ref().map(|x|{
      _fbb.create_vector(x)
    });
    let floats = self.floats.as_ref().map(|x|{
      _fbb.create_vector(x)
    });
    let empty_string = self.empty_string.as_ref().map(|x|{
      _fbb.create_string(x)
    });
    let some_string = self.some_string.as_ref().map(|x|{
      _fbb.create_string(x)
    });
