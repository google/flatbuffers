...
template <typename H>
inline H AbslHashValue(H h, const NestedStruct& obj) {
  for (auto element : *obj.a()) {
    h = H::combine(std::move(h), element);
  }
  for (auto element : *obj.c()) {
    h = H::combine(std::move(h), element);
  }
  for (auto element : *obj.d()) {
    h = H::combine(std::move(h), element);
  }
  return H::combine(std::move(h), obj.b());
}
...
template <typename H>
inline H AbslHashValue(H h, const ArrayStruct& obj) {
  for (auto element : *obj.b()) {
    h = H::combine(std::move(h), element);
  }
  for (const auto* element : *obj.d()) {
    h = H::combine(std::move(h), *element);
  }
  for (auto element : *obj.f()) {
    h = H::combine(std::move(h), element);
  }
  return H::combine(std::move(h), obj.a(), obj.c(), obj.e());
}
