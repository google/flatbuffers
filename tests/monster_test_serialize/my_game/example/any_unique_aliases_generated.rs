...
impl Serialize for AnyUniqueAliases {
    ...
    where
        S: Serializer,
    {
        serializer.serialize_unit_variant("AnyUniqueAliases", u32::from(self.0), self.variant_name().unwrap())
    }
}
...
impl<'de> serde::Deserialize<'de> for AnyUniqueAliases {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        for item in Self::ENUM_VALUES {
            if item.variant_name() == Some(s.as_str()) {
                return Ok(*item);
            }
        }
...
impl AnyUniqueAliasesT {
    #[must_use]
    pub const fn any_unique_aliases_type(&self) -> AnyUniqueAliases {
        match self {
