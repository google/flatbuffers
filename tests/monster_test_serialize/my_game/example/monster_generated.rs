...
impl<'a> Monster<'a> {
    ...
    pub fn unpack(&self) -> MonsterT {
        ...
        let inventory = self.inventory().map(|x| x.to_vec());
        ...
        let testnestedflatbuffer = self.testnestedflatbuffer().map(|x| x.to_vec());
        ...
        let testarrayofbools = self.testarrayofbools().map(|x| x.to_vec());
        ...
        let testarrayofsortedstruct = self
            ...
            .map(|x| x.iter().map(|t| t.unpack()).collect());
        let flex = self.flex().map(|x| x.to_vec());
        ...
        let testrequirednestedflatbuffer = self.testrequirednestedflatbuffer().map(|x| x.to_vec());
    ...
    pub fn any_unique_as_ts(&self) -> Option<TestSimpleTableWithEnum<'a>> {
        if self.any_unique_type() == AnyUniqueAliases::TS {
            self.any_unique()
                .map(TestSimpleTableWithEnum::init_from_table)
        } else {
            None
        }
    }
    ...
    pub fn any_unique_as_m2(&self) -> Option<super::example_2::Monster<'a>> {
        if self.any_unique_type() == AnyUniqueAliases::M2 {
            self.any_unique()
                .map(super::example_2::Monster::init_from_table)
        } else {
            None
        }
    }
