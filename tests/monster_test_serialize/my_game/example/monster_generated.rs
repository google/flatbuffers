...
impl Serialize for Monster<'_> {
  ...
  where
    S: Serializer,
  {
    let mut s = serializer.serialize_struct("Monster", 66)?;
      ...
      s.serialize_field("value_member_type", &self.value_member_type())?;
      match self.value_member_type() {
        Value::NONE => (),
          Value::Monster => {
            let f = self.value_member_as_monster()
              .expect("Invalid union table, expected `Value::Monster`.");
            s.serialize_field("value_member", &f)?;
          }
        _ => unimplemented!(),
      }
      ...
      match self.value_type() {
        Any::NONE => (),
          Any::Monster => {
            let f = self.value_as_monster()
              .expect("Invalid union table, expected `Any::Monster`.");
            s.serialize_field("value", &f)?;
          }
          Any::TestSimpleTableWithEnum => {
            let f = self.value_as_test_simple_table_with_enum()
              .expect("Invalid union table, expected `Any::TestSimpleTableWithEnum`.");
            s.serialize_field("value", &f)?;
          }
          Any::MyGame_Example2_Monster => {
            let f = self.value_as_my_game_example_2_monster()
              .expect("Invalid union table, expected `Any::MyGame_Example2_Monster`.");
            s.serialize_field("value", &f)?;
          }
        _ => unimplemented!(),
