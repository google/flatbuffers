...
final class FlatBuffersUnionTests: XCTestCase {
  ...
  func testUnionVector() {
    ...
    let end = Movie.createMovie(
      &fb,
      vectorOfCharactersType: types,
      vectorOfCharacters: characterVector)
    ...
    XCTAssertEqual(
      ...
      (objc?.characters[0]?.value as? BookReaderT)?.booksRead)
    fb.clear()
    ...
    XCTAssertEqual(
      packedMovie.characters(at: 0, type: BookReader.self)?.booksRead,
      movie.characters(at: 0, type: BookReader.self)?.booksRead)
    XCTAssertEqual(
    ...
    XCTAssertEqual(
      packedMovie.characters(at: 2, type: BookReader.self)?.booksRead,
      movie.characters(at: 2, type: BookReader.self)?.booksRead)
  }
}
