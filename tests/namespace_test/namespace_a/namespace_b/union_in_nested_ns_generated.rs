...
pub enum UnionInNestedNST {
    NONE,
    TableInNestedNS(alloc::boxed::Box<super::TableInNestedNST>),
}
...
impl UnionInNestedNST {
    ...
    /// If the union variant matches, return the owned super::TableInNestedNST, setting the union to NONE.
    pub fn take_table_in_nested_ns(&mut self) -> Option<alloc::boxed::Box<super::TableInNestedNST>> {
    ...
    pub fn as_table_in_nested_ns(&self) -> Option<&super::TableInNestedNST> {
        if let Self::TableInNestedNS(v) = self { Some(v.as_ref()) } else { None }
    }

    /// If the union variant matches, return a mutable reference to the super::TableInNestedNST.
