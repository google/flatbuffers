...
void MiniReflectBoundsTest() {
  std::vector<uint8_t> buf = {
      0x18, 0x00, 0x00, 0x00, 0x00, 0x00, 0x12, 0x00, 0x10, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x08, 0x00, 0xff, 0x00,
      0x0c, 0x00, 0x07, 0x00, 0x12, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x14, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00,
      0x08, 0x00, 0x00, 0x00, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05,
      0x06, 0x07, 0x03, 0x00, 0x00, 0x00, 0x4f, 0x72, 0x63, 0x00,
  };

  flatbuffers::Verifier verifier(buf.data(), buf.size());
  TEST_EQ(verifier.VerifyBuffer<MyGame::Sample::Monster>(), true);

  auto s = flatbuffers::FlatBufferToString(
      buf.data(), buf.size(), MyGame::Sample::Monster::MiniReflectTypeTable());
  TEST_EQ(!s.empty(), true);
}
