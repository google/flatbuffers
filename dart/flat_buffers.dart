...
///
/// Provides an interface for interacting with these builders.
...
class Builder {
  ...
  /// Start a new Struct vector (list).  Must be finished with with [endStructVector].
  ///
  /// Most callers should prefer [writeListOfStructs].
  void startStructVector(int size, int count, int align) {
    _ensureNoVTable();
    _prepare(_sizeofUint32, size * count);
    _prepare(align, size * count);
  }

  /// Finish a Struct vector.  Must be called after writing a structed started with [startStructVector].
  ///
  /// Most callers should prefer [writeListOfStructs].
  int endStructVector(int count) {
    _setUint32AtTail(_buf, _tail, count);
    return _tail;
  }

  int writeListOfStructs(
      List<BuilderHelper> structBuilders, int size, int align) {
    startStructVector(size, structBuilders.length, align);
    for (int i = 0; i < structBuilders.length; i++) {
  ...
  /// Prepares for writing an element of `size` after `additionalBytes`
  /// are written.  Used for writing structs inline.
  void prep(int size, int additionalBytes) {
    _prepare(size, 1, additionalBytes: additionalBytes);
  }

  void pad(int size) {
    _tail -= size;
    for (int i = 0; i < size; i++) {
      _buf.setUint8(_tail + i, 0);
    }
  }
  ...
  void putBool(bool x) {
    _setUint8AtTail(_buf, _tail, x ? 1 : 0);
  }

  void putUint8(int x) {
    _setUint8AtTail(_buf, _tail, x);
  }

  void putInt8(int x) {
    _setInt8AtTail(_buf, _tail, x);
  }

  void putUint16(int x) {
    _setUint16AtTail(_buf, _tail, x);
  }

  void putInt16(int x) {
    _setInt16AtTail(_buf, _tail, x);
  }

  void putUint32(int x) {
    _setUint32AtTail(_buf, _tail, x);
  }

  void putInt32(int x) {
    _setInt32AtTail(_buf, _tail, x);
  }

  void putUint64(int x) {
    _setUint64AtTail(_buf, _tail, x);
  }

  void putInt64(int x) {
    _setInt64AtTail(_buf, _tail, x);
  }

  void putFloat32(double x) {
    _setFloat32AtTail(_buf, _tail, x);
  }

  void putFloat64(double x) {
    _setFloat64AtTail(_buf, _tail, x);
  }
...
class _VTable {
  ...
  /// Outputs this VTable to [buf], which is is expected to be -ed to 16-bit
