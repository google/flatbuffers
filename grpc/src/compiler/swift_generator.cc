...
static std::string GenerateInputMethodTypes(const grpc_generator::Method* method) {
...
static std::string GenerateOutputMethodTypes(const grpc_generator::Method* method) {
  if (method->BidiStreaming()) {
...
void GenerateCoders(const grpc_generator::Service* service,
  ...
  printer->Print("do {\n");
  printer->Indent();
  printer->Print("return try self.serialize(message: message) { GRPCNIOTransportBytes($0) } as! Bytes\n");
  printer->Outdent();
...
void GenerateServiceProtocols(const grpc_generator::Service* service,
  ...
  for (auto it = 0; it < service->method_count(); it++) {
    ...
    vars["Input"] = GenerateType(QualifiedName(method->get_input_namespace_parts(), method->get_output_type_name()), GenerateInputMethodTypes(&*method));

    vars["Output"] = GenerateType(QualifiedName(method->get_input_namespace_parts(), method->get_output_type_name()), GenerateOutputMethodTypes(&*method));
    auto name = method->name();
    vars["MethodName"] = name;
    GenerateFunction(printer, &vars);
  }
  ...
  // Simple service
  printer->Print(vars, "$ACCESS$ protocol SimpleServiceProtocol: $SwiftServiceQualifiedName$.ServiceProtocol {\n");
  printer->Indent();

  for (auto it = 0; it < service->method_count(); it++) {
    auto method = service->method(it);

    vars["Input"] = GenerateGRPCMessage(QualifiedName(method->get_input_namespace_parts(), method->get_output_type_name()));
    ...
    if (method->ClientStreaming() || method->BidiStreaming()) {
      printer->Print(vars, "request: GRPCCore.RPCAsyncSequence<$Input$, any Swift.Error>,\n");
    } else {
      printer->Print(vars, "request: $Input$,\n");
    }
...
void CreateServiceProtocolFunctionsImplementations(grpc_generator::Printer* printer,
                    std::map<grpc::string, grpc::string>* dictonary) {
  auto vars = *dictonary;
...
void GenerateServiceDefaultImplementation(const grpc_generator::Service* service,
                              grpc_generator::Printer* printer,
                              std::map<grpc::string, grpc::string>* dictonary) {
  auto vars = *dictonary;
  EnforceOSVersion(printer);
...
void GenerateSimpleServiceImplemetation(const grpc_generator::Service* service,
                              grpc_generator::Printer* printer,
                              std::map<grpc::string, grpc::string>* dictonary) {
  auto vars = *dictonary;
  EnforceOSVersion(printer);
  printer->Print(
                 vars,
                 "extension $SwiftServiceQualifiedName$.SimpleServiceProtocol {\n");
  printer->Indent();
...
void GenerateClientProtocols(const grpc_generator::Service* service,
  ...
  for (auto it = 0; it < service->method_count(); it++) {
    ...
    vars["serializer"] = GenerateType(QualifiedName(method->get_input_namespace_parts(), method->get_output_type_name()), "GRPCCore.MessageSerializer");

    vars["deserializer"] = GenerateType(QualifiedName(method->get_input_namespace_parts(), method->get_output_type_name()), "GRPCCore.MessageDeserializer");

    if (method->ClientStreaming() || method->BidiStreaming()) {
      vars["request"] = GenerateType(QualifiedName(method->get_input_namespace_parts(), method->get_output_type_name()), "GRPCCore.StreamingClientRequest");
    } else {
      vars["request"] = GenerateType(QualifiedName(method->get_input_namespace_parts(), method->get_output_type_name()), "GRPCCore.ClientRequest");
    }

    if (method->ServerStreaming() || method->BidiStreaming()) {
      vars["response"] = GenerateType(QualifiedName(method->get_input_namespace_parts(), method->get_output_type_name()), "GRPCCore.StreamingClientResponse");
    } else {
      vars["response"] = GenerateType(QualifiedName(method->get_input_namespace_parts(), method->get_output_type_name()), "GRPCCore.ClientResponse");
    }
...
void GenerateClientStruct(const grpc_generator::Service* service,
  ...
  printer->Indent();
  // Base protocol
  printer->Print(vars, "$ACCESS$ struct Client<Transport>: ClientProtocol where Transport: GRPCCore.ClientTransport {\n");
  printer->Indent();
  printer->Print("private let client: GRPCCore.GRPCClient<Transport>\n\n");
  printer->Print(vars, "$ACCESS$ init(wrapping client: GRPCCore.GRPCClient<Transport>) {\n");
  printer->Indent();
...
void GenerateClientDefaultImplementation(const grpc_generator::Service* service,
                              grpc_generator::Printer* printer,
                              std::map<grpc::string, grpc::string>* dictonary) {
  auto vars = *dictonary;
  EnforceOSVersion(printer);
  ...
  for (auto it = 0; it < service->method_count(); it++) {
    ...
    vars["serializer"] = GenerateType(QualifiedName(method->get_input_namespace_parts(), method->get_output_type_name()), "FlatBuffersMessageSerializer");

    vars["deserializer"] = GenerateType(QualifiedName(method->get_input_namespace_parts(), method->get_output_type_name()), "FlatBuffersMessageDeserializer");
...
void GenerateClientHelperMethods(const grpc_generator::Service* service,
                              grpc_generator::Printer* printer,
                              std::map<grpc::string, grpc::string>* dictonary) {
  auto vars = *dictonary;
  ...
  for (auto it = 0; it < service->method_count(); it++) {
    ...
    if (method->NoStreaming() || method->ServerStreaming()) {
      vars["request"] = GenerateGRPCMessage(QualifiedName(method->get_input_namespace_parts(), method->get_output_type_name()));
    ...
    if (method->ClientStreaming() || method->BidiStreaming()) {
      vars["request"] = GenerateGRPCMessage(QualifiedName(method->get_input_namespace_parts(), method->get_output_type_name()));
      printer->Print(vars, "requestProducer producer: @Sendable @escaping (GRPCCore.RPCWriter<$request$>) async throws -> Void,\n");
    }


    if (method->NoStreaming() || method->ClientStreaming()) {
      vars["response"] = GenerateGRPCMessage(QualifiedName(method->get_input_namespace_parts(), method->get_output_type_name()));

      printer->Print(vars, "onResponse handleResponse: @Sendable @escaping (GRPCCore.ClientResponse<$response$>) async throws -> Result = { try $0.message }\n");
    } else {
      vars["response"] = GenerateGRPCMessage(QualifiedName(method->get_input_namespace_parts(), method->get_output_type_name()));

      printer->Print(vars, "onResponse handleResponse: @Sendable @escaping (GRPCCore.StreamingClientResponse<$response$>) async throws -> Result\n");
    }
