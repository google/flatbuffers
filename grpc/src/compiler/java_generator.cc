...
namespace grpc_java_generator {
    ...
    void GenerateImports(grpc_generator::File *file, grpc_generator::Printer *printer, VARS& vars) {
        vars["filename"] = file->filename();
        printer->Print(vars, "//Generated by flatc compiler (version $flatc_version$) on " __DATE__  " " __TIME__ "  \n");
    ...
    static void GrpcWriteDocComment(Printer* printer, VARS &vars, const string& comments) {
        printer->Print("/**\n");
    ...
    static void PrintMethodFields(Printer* p, VARS& vars,
        ...
        for (int i = 0; i < service->method_count(); ++i) {
            ...
            p->Print(
                     ...
                     "\n\n"
                     "private static volatile FlatbuffersUtils.FBExtactor<$input_type$> extractorOf$input_type_name$;\n"
                     "private static FlatbuffersUtils.FBExtactor<$input_type$> getExtractorOf$input_type_name$() {\n"
                     "    if (extractorOf$input_type_name$ != null) return extractorOf$input_type_name$;\n"
                     "    synchronized ($service_class_name$.class) {\n"
                     "        if (extractorOf$input_type_name$ != null) return extractorOf$input_type_name$;\n"
                     "        extractorOf$input_type_name$ = new FlatbuffersUtils.FBExtactor<$input_type$>() {\n"
                     "            public $input_type$ extract (ByteBuffer buffer) {\n"
                     "                return $input_type$.getRootAs$input_type_name$(buffer);\n"
                     "            }\n"
                     "        };\n"
                     "        return extractorOf$input_type_name$;\n"
                     "    }\n"
                     "}\n\n"
                     "private static volatile FlatbuffersUtils.FBExtactor<$output_type$> extractorOf$output_type_name$;\n"
                     "private static FlatbuffersUtils.FBExtactor<$output_type$> getExtractorOf$output_type_name$() {\n"
                     "    if (extractorOf$output_type_name$ != null) return extractorOf$output_type_name$;\n"
                     "    synchronized ($service_class_name$.class) {\n"
                     "        if (extractorOf$output_type_name$ != null) return extractorOf$output_type_name$;\n"
                     "        extractorOf$output_type_name$ = new FlatbuffersUtils.FBExtactor<$output_type$>() {\n"
                     "            public $output_type$ extract (ByteBuffer buffer) {\n"
                     "                return $output_type$.getRootAs$output_type_name$(buffer);\n"
                     "            }\n"
                     "        };\n"
...
//            vars["proto_method_descriptor_supplier"] = service->name() + "MethodDescriptorSupplier";
    ...
    static void PrintStub(Printer* p, VARS& vars,
                          const ServiceDescriptor* service, StubType type) {
    ...
    static bool CompareMethodClientStreaming(const std::unique_ptr<const grpc_generator::Method>& method1,
    ...
    static void PrintMethodHandlerClass(Printer* p, VARS& vars,
        ...
        std::vector<std::unique_ptr<const grpc_generator::Method>> sorted_methods(service->method_count());
        for (int i = 0; i < service->method_count(); ++i) {
    ...
    static void PrintGetServiceDescriptorMethod(Printer* p,
                                                VARS& vars,
                                                const ServiceDescriptor* service) {
        vars["service_name"] = service->name();
...
//                 "}\n\n");
    ...
    static void PrintService(Printer* p, VARS& vars,
        ...
        GrpcWriteDocComment(p, vars, " Creates a new async stub that supports all call types for the service");
        p->Print(
                 vars,
        ...
        // TODO(nmittler): Replace with WriteDocComment once included by protobuf distro.
        GrpcWriteDocComment(p, vars, " Creates a new blocking-style stub that supports unary and streaming "
        ...
        // TODO(nmittler): Replace with WriteDocComment once included by protobuf distro.
        GrpcWriteDocComment(p, vars, " Creates a new ListenableFuture-style stub that supports unary calls "
                            "on the service");
        p->Print(
                 vars,
...
void GenerateService(const grpc_generator::Service *service,
        ...
        vars["ProtoFileDescriptorSupplier"] = "io.grpc.protobuf.ProtoFileDescriptorSupplier";
        vars["ProtoServiceDescriptorSupplier"] = "io.grpc.protobuf.ProtoServiceDescriptorSupplier";
        vars["ProtoMethodDescriptorSupplier"] = "io.grpc.protobuf.ProtoMethodDescriptorSupplier";
