...
exports.Verifier = exports.TraceKind = exports.ErrorKind = void 0;
/**
...
(function (ErrorKind) {
     ...
     * malicious buffers.
     */
    ErrorKind["DepthLimitReached"] = "DepthLimitReached";
    /**
     ...
     */
    ErrorKind["TooManyTables"] = "TooManyTables";
    /**
     ...
     */
    ErrorKind["ApparentSizeTooLarge"] = "ApparentSizeTooLarge";
     ...
     */
    ErrorKind["InconsistentUnion"] = "InconsistentUnion";
    /**
     ...
     */
    ErrorKind["InvalidVtable"] = "InvalidVtable";
    /**
     ...
     */
    ErrorKind["InvalidVectorLength"] = "InvalidVectorLength";
    /**
     ...
     */
    ErrorKind["SignedOffsetOutOfBounds"] = "SignedOffsetOutOfBounds";
...
(function (TraceKind) {
    ...
    TraceKind["TableField"] = "TableField";
    /** The trace entry refers to a positional element inside a vector. */
    TraceKind["VectorElement"] = "VectorElement";
    /** The trace entry refers to a typed variant of a union field. */
    TraceKind["UnionVariant"] = "UnionVariant";
})(TraceKind || (exports.TraceKind = TraceKind = {}));
const DEFAULT_MAX_DEPTH = 64;
const DEFAULT_MAX_TABLES = 1000000;
const DEFAULT_MAX_APPARENT_SIZE = 1073741824;
/**
...
class Verifier {
     ...
     */
    readUint32(pos) {
        this.checkRange(pos, constants_js_1.SIZEOF_INT);
     ...
     */
    checkVtable(tablePos) {
        this.checkRange(tablePos, constants_js_1.SIZEOF_INT);
        ...
        this.checkRange(vtablePos, constants_js_1.SIZEOF_SHORT);
        const vtableLen = this.buf.getUint16(vtablePos, true);
        if (vtableLen < 2 * constants_js_1.SIZEOF_SHORT || vtableLen % constants_js_1.SIZEOF_SHORT !== 0) {
            throw {
    ...
    checkString(pos) {
        ...
        this.checkRange(strStart, constants_js_1.SIZEOF_INT);
        const strLen = this.buf.getUint32(strStart, true);
        const dataStart = strStart + constants_js_1.SIZEOF_INT;
        this.checkRange(dataStart, strLen + 1);
        const bytes = new Uint8Array(this.buf.buffer, this.buf.byteOffset + dataStart, strLen);
        try {
     ...
     */
    checkVector(pos, elementSize) {
        this.checkRange(pos, constants_js_1.SIZEOF_INT);
        const vecOffset = this.buf.getInt32(pos, true);
        const vecStart = pos + vecOffset;
        this.checkRange(vecStart, constants_js_1.SIZEOF_INT);
        const length = this.buf.getUint32(vecStart, true);
        const dataStart = vecStart + constants_js_1.SIZEOF_INT;
     ...
     */
    checkVectorOfTables(pos, verifyElem) {
        const length = this.checkVector(pos, constants_js_1.SIZEOF_INT);
        this.checkRange(pos, constants_js_1.SIZEOF_INT);
        const vecOffset = this.buf.getInt32(pos, true);
        const vecStart = pos + vecOffset;
        const dataStart = vecStart + constants_js_1.SIZEOF_INT;
        for (let i = 0; i < length; i++) {
            const elemPos = dataStart + i * constants_js_1.SIZEOF_INT;
            // Each element is a relative offset to the target table.
            this.checkRange(elemPos, constants_js_1.SIZEOF_INT);
     ...
     *   `_resolveVtableField`.
     */
    checkScalarField(tablePos, vOffset, size) {
     ...
     *   `_resolveVtableField`.
     */
    checkOffsetField(tablePos, vOffset) {
     ...
     *   `_resolveVtableField`.
     */
    checkRequiredField(tablePos, vOffset, fieldName) {
        const fieldPos = this._resolveVtableField(tablePos, vOffset);
        if (fieldPos === 0) {
            throw {
     ...
     */
    checkUnionConsistency(tablePos, typeVOffset, valueVOffset, fieldName) {
     ...
     */
    readFieldUint8(tablePos, vOffset) {
        const fieldPos = this._resolveVtableField(tablePos, vOffset);
        if (fieldPos === 0) {
            return 0;
        }
    ...
    _resolveVtableField(tablePos, vOffset) {
        this.checkRange(tablePos, constants_js_1.SIZEOF_INT);
        const soffset = this.buf.getInt32(tablePos, true);
        const vtablePos = tablePos - soffset;
        this.checkRange(vtablePos, constants_js_1.SIZEOF_SHORT);
        ...
        this.checkRange(vtablePos + vOffset, constants_js_1.SIZEOF_SHORT);
