...
{
  LoadFileFunction load_file_function= g_load_file_function ? g_load_file_function : LoadFileRaw;
  FLATBUFFERS_ASSERT(load_file_function);

  std::string buf;
  if (!g_load_file_function(name, false, &buf)) return false;

  std::stringstream istr(buf);
  std::string line; 
  while (!std::getline(istr, line).eof()) {
