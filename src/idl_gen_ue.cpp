...
static inline std::string NumToStringUe(std::string val, BaseType type) {
...
static std::string GenIncludeGuard() {
  return "#pragma once\n";
}
...
struct IDLOptionsUe : public IDLOptions {
  ...
  IDLOptionsUe(const IDLOptions &opts)
...
class UeGenerator : public BaseGenerator {
 public:
  UeGenerator(const Parser &parser, const std::string &path,
               const std::string &file_name, IDLOptionsUe opts)
 ...
 private:
  ...
  bool VectorElementUserFacing(const Type &type) const {
    return (opts_.scoped_enums && IsEnum(type)) ||
           (opts_.g_cpp_std >= ue::CPP_STD_17 && opts_.g_only_fixed_enums &&
            IsEnum(type));
  }
  ...
  void GenMiniReflect(const StructDef *struct_def, const EnumDef *enum_def) {
    ...
    if (enum_def && !consecutive_enum_from_zero) {
      for (auto it = enum_def->Vals().begin(); it != enum_def->Vals().end();
        ...
        const auto &ev = **it;
        if (!vs.empty()) vs += ", ";
        vs += NumToStringUe(enum_def->ToString(ev),
  ...
  void GenEnum(const EnumDef &enum_def) {
    ...
    for (const auto ev : enum_def.Vals()) {
      ...
      code_.SetValue("VALUE",
                     NumToStringUe(enum_def.ToString(*ev),
                                    enum_def.underlying_type.base_type));
      code_ += "  {{KEY}} = {{VALUE}}\\";
      add_sep = true;
    }
    ...
    if (opts_.scoped_enums || opts_.prefixed_enums) {
      ...
      if (enum_def.attributes.Lookup("bit_flags")) {
        ...
        code_.SetValue("VALUE",
                       NumToStringUe(enum_def.AllFlags(),
                                      enum_def.underlying_type.base_type));
        code_ += "{{SEP}}  {{KEY}} = {{VALUE}}\\";
  ...
  void GenUnionPost(const EnumDef &enum_def) {
    ...
    if (opts_.generate_object_based_api) {
      ...
      for (const auto &ev : enum_def.Vals()) {
        ...
        code_ += "    case {{LABEL}}: {";
        bool copyable = true;
        if (opts_.g_cpp_std < ue::CPP_STD_11 &&
  ...
  std::string GenDefaultConstant(const FieldDef &field) {
    if (IsFloat(field.value.type.base_type))
      ...
      return NumToStringUe(field.value.constant, field.value.type.base_type);
  }
    ...
    if (field.IsScalarOptional()) {
      ...
      if (ev) {
        ...
        return GenUnderlyingCast(
            field, true, NumToStringUe(field.value.constant, type.base_type));
      }
  ...
  void GenCopyMoveCtorAndAssigOpDecls(const StructDef &struct_def) {
    if (opts_.g_cpp_std < ue::CPP_STD_11) return;
  ...
  void GenBuilders(const StructDef &struct_def) {
    ...
    if (opts_.g_cpp_std >= ue::CPP_STD_17) { GenTraitsStruct(struct_def); }
  ...
  void GenTablePost(const StructDef &struct_def) {
    ...
    if (opts_.generate_object_based_api) {
      ...
      if (opts_.g_cpp_std == ue::CPP_STD_X0) {
      ...
      } else if (opts_.g_cpp_std == ue::CPP_STD_11) {
        code_ +=
...
static bool GenerateCPP(const Parser &parser, const std::string &path,
  ...
  ue::IDLOptionsUe opts(parser.opts);
  ...
  if (opts.cpp_static_reflection && opts.g_cpp_std < ue::CPP_STD_17) {
    LogCompilerError(
  ...
  ue::UeGenerator generator(parser, path, file_name, opts);
...
static std::string CPPMakeRule(const Parser &parser, const std::string &path,
  ...
  ue::UeGenerator geneartor(parser, path, file_name, parser.opts);
...
class UeCodeGenerator : public CodeGenerator {
 public:
  ...
  IDLOptions::Language Language() const override { return IDLOptions::kUe; }

  std::string LanguageName() const override { return "Ue C++"; }
};

}  // namespace

std::unique_ptr<CodeGenerator> NewUeCodeGenerator() {
  return std::unique_ptr<UeCodeGenerator>(new UeCodeGenerator());
}
