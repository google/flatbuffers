...
class RustGenerator : public BaseGenerator {
 ...
 private:
  ...
  std::string WrapInNameSpace(const Namespace *ns, const std::string &module,
    ...
    return module + joiner + prefix + name;
  }
  ...
  void ForAllUnionVariantsBesidesNone(
    ...
    for (auto it = def.Vals().begin(); it != def.Vals().end(); ++it) {
      ...
      code_.SetValue(
          "U_ELEMENT_ENUM_TYPE",
          WrapInNameSpace(def.defined_namespace, GetModule(def), namer_.EnumVariant(def, ev)));
      code_.SetValue(
          "U_ELEMENT_TABLE_TYPE",
  ...
  void GenNamespaceImports(const int white_spaces) {
    ...
    if (!parser_.opts.generate_all) {
      ...
      for (auto it = included_files.begin(); it != included_files.end(); ++it) {
        ...
        auto module_map = parser_.opts.module_map.schema_to_module.find(it->filename);
        if (module_map != parser_.opts.module_map.schema_to_module.end()) {
          // This include comes from another crate. The fully qualified name will be used in the generated code.
          continue;
        }
