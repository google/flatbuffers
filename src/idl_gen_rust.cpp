...
struct RustPath {
  ...
  bool operator==(const RustPath& rhs) const {
    return components == rhs.components;
  }
  bool operator!=(const RustPath& rhs) const { return !(*this == rhs); }

  RustPath DropFirst() const {
    if (components.empty()) return RustPath();
    RustPath dropped;
    dropped.components.assign(components.begin() + 1, components.end());
    return dropped;
  }
  ...
  static RustPath ParseRustModule(const std::string& rust_module) {
    RustPath path;
    size_t begin = 0;
    while (begin <= rust_module.size()) {
      const size_t end = rust_module.find("::", begin);
      const std::string component = rust_module.substr(
  ...
  static RustPath ParseRustModule(const std::string& rust_module,
                                  const Namer& namer) {
    RustPath path;
    const RustPath raw = ParseRustModule(rust_module);
    path.components.reserve(raw.components.size());
    for (auto it = raw.components.begin(); it != raw.components.end(); ++it) {
      path.components.push_back(namer.Namespace(*it));
    }
    return path;
  }

  static RustPath FromNamespace(const Namespace* ns, const Namer& namer) {
    RustPath path;
    if (!ns) {
      return path;
    }
    path.components.reserve(ns->components.size());
...
static RustPath GetEffectiveModulePath(const Parser& parser, const Namer& namer,
                                       const Definition& symbol) {
  const auto rust_module_it = parser.rust_modules_by_file_.find(symbol.file);
  if (rust_module_it != parser.rust_modules_by_file_.end() &&
      !rust_module_it->second.empty()) {
    RustPath path = RustPath::ParseRustModule(rust_module_it->second, namer);
    // The first rust_module component is the crate name; generated modules are
    // nested inside that crate.
    return path.DropFirst();
  }

  return RustPath::FromNamespace(symbol.defined_namespace, namer);
}
}  // namespace
...
static bool GenerateRustModuleRootFile(const Parser& parser,
  ...
  struct Module {
    ...
    void Insert(const Namer& namer, const RustPath& module_path,
                const Definition* s) {
      const Definition& symbol = *s;
      Module* current_module = this;
      for (auto it = module_path.components.begin();
           it != module_path.components.end(); it++) {
        current_module = &current_module->sub_modules[*it];
      }
      current_module->generated_files.push_back(
          namer.File(symbol.name, SkipFile::Extension));
    }

    // Recursively create mod.rs files from the module tree.
    bool GenerateModFiles(const Parser& parser, const std::string& output_dir) {
      CodeWriter code("    ");
      ...
      const bool save_success = parser.opts.file_saver->SaveFile(
      ...
      for (auto it = sub_modules.begin(); it != sub_modules.end(); it++) {
        const std::string submodule_dir =
            output_dir + it->first + kPathSeparator;
        EnsureDirExists(submodule_dir);
        if (!it->second.GenerateModFiles(parser, submodule_dir)) {
          return false;
        }
      }
  ...
  for (auto it = parser.enums_.vec.begin(); it != parser.enums_.vec.end();
       it++) {
    root_module.Insert(namer, GetEffectiveModulePath(parser, namer, **it), *it);
  }
  ...
  return root_module.GenerateModFiles(parser, output_dir);
}
...
class RustGenerator : public BaseGenerator {
 public:
  RustGenerator(const Parser& parser, const std::string& path,
      ...
      : BaseGenerator(parser, path, file_name, "", "::", "rs"),
        current_definition_has_rust_module_(false),
        namer_(WithFlagOptions(RustDefaultConfig(), parser.opts, path),
               RustKeywords()) {
    for (auto it = parser_.rust_modules_by_file_.begin();
         it != parser_.rust_modules_by_file_.end(); ++it) {
      RustPath path_components = RustPath::ParseRustModule(it->second, namer_);
      rust_module_path_by_file_[it->first] = path_components;
    }
  ...
  bool GenerateSymbols(const SymbolTable<T>& symbols,
    ...
    for (auto it = symbols.vec.begin(); it != symbols.vec.end(); it++) {
      ...
      cur_module_path_ = GetLocalModulePath(symbol);
      current_rust_module_path_ = GetRustModulePath(symbol);
      current_definition_has_rust_module_ = HasRustModule(symbol);
      gen_symbol(symbol);

      const std::string directories = GetModuleOutputDir(cur_module_path_);
  ...
  bool GenerateOneFile() {
    ...
    FLATBUFFERS_ASSERT(cur_module_path_.empty());

    // Generate imports for the global scope in case no namespace is used
    // in the schema file.
    GenNamespaceImports();

    // Generate all code in their modules, once, because Rust does not
    // permit re-opening modules.
    std::vector<RustPath> module_paths;
    auto append_path = [&](const Definition& def) {
      const auto path = GetLocalModulePath(def);
      if (std::find(module_paths.begin(), module_paths.end(), path) ==
          module_paths.end()) {
    ...
    for (auto it = parser_.enums_.vec.begin(); it != parser_.enums_.vec.end();
         ++it) {
      if (!(*it)->generated) append_path(**it);
    }
    for (auto it = parser_.structs_.vec.begin();
         it != parser_.structs_.vec.end(); ++it) {
      if ((*it)->fixed && !(*it)->generated) append_path(**it);
    }
    for (auto it = parser_.structs_.vec.begin();
         it != parser_.structs_.vec.end(); ++it) {
      if (!(*it)->fixed && !(*it)->generated) append_path(**it);
    ...
    for (auto path_it = module_paths.begin(); path_it != module_paths.end();
         ++path_it) {
      const auto& module_path = *path_it;

      // Generate code for all the enum declarations.
      for (auto it = parser_.enums_.vec.begin(); it != parser_.enums_.vec.end();
           ++it) {
        const auto& enum_def = **it;
        if (GetLocalModulePath(enum_def) == module_path &&
            !enum_def.generated) {
          SetNameSpace(module_path);
          current_rust_module_path_ = GetRustModulePath(enum_def);
          current_definition_has_rust_module_ = HasRustModule(enum_def);
          GenEnum(enum_def);
        }
      }
      ...
      for (auto it = parser_.structs_.vec.begin();
        ...
        if (GetLocalModulePath(struct_def) == module_path && struct_def.fixed &&
            !struct_def.generated) {
          SetNameSpace(module_path);
          current_rust_module_path_ = GetRustModulePath(struct_def);
          current_definition_has_rust_module_ = HasRustModule(struct_def);
      ...
      for (auto it = parser_.structs_.vec.begin();
           it != parser_.structs_.vec.end(); ++it) {
        const auto& struct_def = **it;
        if (GetLocalModulePath(struct_def) == module_path &&
            !struct_def.fixed && !struct_def.generated) {
          SetNameSpace(module_path);
          current_rust_module_path_ = GetRustModulePath(struct_def);
          current_definition_has_rust_module_ = HasRustModule(struct_def);
          GenTable(struct_def);
    ...
    if (!cur_module_path_.empty()) SetNameSpace(RustPath());
 ...
 private:
  ...
  // This tracks the current module path so we can insert module declarations.
  RustPath cur_module_path_;
  RustPath current_rust_module_path_;
  bool current_definition_has_rust_module_;
  std::map<std::string, RustPath> rust_module_path_by_file_;

  const RustPath& CurrentModulePath() const { return cur_module_path_; }

  bool HasRustModule(const Definition& def) const {
    const auto it = rust_module_path_by_file_.find(def.file);
    return it != rust_module_path_by_file_.end() && !it->second.empty();
  }

  RustPath GetRustModulePath(const Definition& def) const {
    const auto it = rust_module_path_by_file_.find(def.file);
    if (it != rust_module_path_by_file_.end()) {
      return it->second;
    }
    return RustPath();
  }

  RustPath NamespaceModulePath(const Namespace* ns) const {
    return RustPath::FromNamespace(ns, namer_);
  }

  RustPath GetLocalModulePath(const Definition& def) const {
    const auto it = rust_module_path_by_file_.find(def.file);
    if (it != rust_module_path_by_file_.end() && !it->second.empty()) {
      return it->second.DropFirst();
    }
    return NamespaceModulePath(def.defined_namespace);
  }

  RustPath GetExternalModulePath(const Definition& def) const {
    const auto it = rust_module_path_by_file_.find(def.file);
    if (it != rust_module_path_by_file_.end() && !it->second.empty()) {
      return it->second;
    }
  ...
  std::string GetModuleOutputDir(const RustPath& module_path) const {
    std::string directories = path_;
    for (auto it = module_path.components.begin();
         it != module_path.components.end(); ++it) {
      directories += *it;
      directories += kPathSeparator;
    }
    return directories;
  }
  ...
  std::string NamespacedNativeName(const EnumDef& def) {
    return WrapInNameSpace(def, namer_.ObjectType(def));
  }
  ...
  std::string WrapInNameSpace(const Definition& def) const {
    return WrapInNameSpace(def, namer_.EscapeKeyword(def.name));
  }
  std::string WrapInNameSpace(const Definition& def,
                              const std::string& name) const {
    const std::string prefix = GetPathPrefixForDefinition(def);
    return prefix.empty() ? name : prefix + name;
  }
  std::string WrapInNameSpace(const Namespace* ns,
                              const std::string& name) const {
    const std::string prefix = GetPathPrefixForNamespace(ns);
    return prefix.empty() ? name : prefix + name;
  }

  std::string GetPathPrefixForDefinition(const Definition& def) const {
    const auto it = rust_module_path_by_file_.find(def.file);
    const bool has_rust_module =
        it != rust_module_path_by_file_.end() && !it->second.empty();

    if (has_rust_module) {
      if (it->second != current_rust_module_path_) {
        return "::" + it->second.Join() + "::";
      }
      const auto local = it->second.DropFirst().Join();
      return local.empty() ? "crate::" : "crate::" + local + "::";
    }

    return GetPathPrefixForNamespace(def.defined_namespace);
  }

  std::string GetPathPrefixForNamespace(const Namespace* ns) const {
    const auto dst = NamespaceModulePath(ns);

    if (current_definition_has_rust_module_) {
      const auto local = dst.Join();
      return local.empty() ? "crate::" : "crate::" + local + "::";
    }

    if (!parser_.opts.rust_module_root_file && CurrentModulePath() == dst) {
      return "";
    }
    if (parser_.opts.rust_module_root_file) {
      return "super::" +
             GetRelativeNamespaceTraversal(CurrentModulePath(), dst);
    }
    return GetRelativeNamespaceTraversal(CurrentModulePath(), dst);
  }

  // Determine the relative module traversal needed to reference one module
  // from another module. This is useful because it does not force
  // the user to have a particular file layout. (If we output absolute
  // module paths, that may require users to organize their Rust crates in a
  // particular way.)
  std::string GetRelativeNamespaceTraversal(const RustPath& src,
                                            const RustPath& dst) const {
    ...
    size_t common = 0;
    while (common < src.components.size() && common < dst.components.size() &&
           src.components[common] == dst.components[common])
      common++;
    for (size_t i = common; i < src.components.size(); i++) stream << "super::";
    for (size_t i = common; i < dst.components.size(); i++) {
      stream << dst.components[i] + "::";
    }
    return stream.str();
  }
    ...
    switch (GetFullType(type)) {
      ...
      case ftTable: {
        return WrapInNameSpace(*type.struct_def,
                               namer_.EscapeKeyword(type.struct_def->name)) +
               "<'a>";
      }
      default: {
        return WrapInNameSpace(*type.struct_def,
  ...
  void ForAllUnionVariantsBesidesNone(
    ...
    for (auto it = def.Vals().begin(); it != def.Vals().end(); ++it) {
      ...
      code_.SetValue("U_ELEMENT_ENUM_TYPE",
                     WrapInNameSpace(def, namer_.EnumVariant(def, ev)));
      code_.SetValue("U_ELEMENT_TABLE_TYPE",
                     WrapInNameSpace(
                         *ev.union_type.struct_def,
                         namer_.EscapeKeyword(ev.union_type.struct_def->name)));
  ...
  // Set up the correct module path. This opens modules if the current path is
  // different from the target path. This function closes and opens modules
  // only as necessary.
  //
  // The file must start and end with an empty path so that modules are
  // properly opened and closed.
  void SetNameSpace(const RustPath& path) {
    if (cur_module_path_ == path) {
      return;
    }

    // Compute the size of the longest common module prefix.
    // If cur_module_path_ is A::B::C::D and path is A::B::E::F::G,
    // the common prefix is A::B:: and we have old_size = 4, new_size = 5
    // and common_prefix_size = 2
    size_t old_size = cur_module_path_.components.size();
    size_t new_size = path.components.size();

    size_t common_prefix_size = 0;
    while (common_prefix_size < old_size && common_prefix_size < new_size &&
           path.components[common_prefix_size] ==
               cur_module_path_.components[common_prefix_size]) {
      common_prefix_size++;
    }

    // Close cur_module_path_ in reverse order to reach the common prefix.
    // In the previous example, D then C are closed.
    for (size_t j = old_size; j > common_prefix_size; --j) {
      code_.DecrementIdentLevel();
      code_ += "} // pub mod " + cur_module_path_.components[j - 1];
    }

    // Open module path components to reach the target path.
    // in the previous example, E, then F, then G are opened
    for (auto j = common_prefix_size; j != new_size; ++j) {
      code_ += "";
      code_ += "#[allow(unused_imports, dead_code)]";
      code_ += "pub mod " + path.components[j] + " {";
      code_.IncrementIdentLevel();
      GenNamespaceImports();
    }

    cur_module_path_ = path;
  }
