...
bool GenerateRustModuleRootFile(const Parser& parser,
  ...
  struct Module {
    ...
    void Insert(const Definition *s, const std::string suffix) {
      ...
      current_module->generated_files.push_back(
        MakeSnakeCase(symbol.name) + suffix
      );
    }
    ...
    void GenerateImports(CodeWriter &code) {
      ...
      for (
        auto it = generated_files.begin(); it != generated_files.end(); it++
      ) {
        code += "pub mod " + *it + ";";
  ...
  Module root_module;
  for (
    auto it = parser.enums_.vec.begin();
    it != parser.enums_.vec.end(); it++
  ) {
    root_module.Insert(*it, parser.opts.filename_suffix);
  }
  for (
    auto it = parser.structs_.vec.begin();
    it != parser.structs_.vec.end(); it++
  ) {
    root_module.Insert(*it, parser.opts.filename_suffix);
  }
  CodeWriter code("    ");
  // DO NOT SUBMIT: CASPER: Move generated warning out of BaseGenerator
  // DO NOT SUBMIT: CASPER: What if we had multiple mod files?
  code += "// Automatically generated by the Flatbuffers compiler. "
          "Do not modify.";
  root_module.GenerateImports(code);
  const bool success = SaveFile((output_dir + "mod.rs").c_str(),
                                code.ToString(), false);
  code.Clear();
  return success;
}
 ...
 public:
  ...
  bool generate() {
    if (parser_.opts.one_file) {
      return generate_one_file();
    } else {
      return generate_individual_files();
    }
  }
  ...
  bool GenerateSymbols(const SymbolTable<T> &symbols,
    ...
    for (auto it = symbols.vec.begin(); it != symbols.vec.end(); it++) {
      ...
      << parser_.opts.filename_suffix << ".rs";
      const bool save_success = SaveFile(file_path.str().c_str(),
                                         code_.ToString(),
                                         /*binary=*/false);
  ...
  // DO NOT SUBMIT: CASPER: CamelCase functions?
  bool generate_individual_files() {
    code_.Clear();
