...
std::string FlatCompiler::GetUsageString(const char *program_name) const {
  ...
  ss <<
    ...
    "  --object-prefix    Customise class prefix for C++/go object-based API.\n"
    "  --object-suffix    Customise class suffix for C++/go object-based API.\n"
    "                     Default value is \"T\".\n"
    ...
    "  --go-enum-sep      String used to separate go enum name and value\n"
    ...
    "  --grpc-object-api  Use the object API for gRPC.\n"
...
int FlatCompiler::Compile(int argc, const char **argv) {
  ...
  for (int argi = 0; argi < argc; argi++) {
    ...
    if (arg[0] == '-') {
      ...
      if (arg == "-o") {
      ...
      } else if (arg == "--go-protobuf-compat") {
        opts.go_protobuf_compat = true;
        opts.receiver_suffix = "Receiver";
        opts.object_suffix = "";
        opts.go_enum_sep = "_";
        opts.go_object_marshal = true;
        ...
        grpc_enabled = true;
      } else if (arg == "--grpc-object-api") {
        opts.grpc_object_api = true;
  ...
  if (opts.go_protobuf_compat) {
    if (opts.lang_to_generate != IDLOptions::kGo) {
      Error("--go-protobuf-compat must only be used with --go.", false, false);
    }
  }
