...
std::string FlatCompiler::GetUsageString(const char *program_name) const {
  ...
  ss <<
	...
	"  --file-names FILE  Specify a file as FILE input. Reads each line from the\n"
	"                     file instead of specifying each as a FILE argument.\n"
  ...
  for (int argi = 0; argi < argc; argi++) {
    ...
    if (arg[0] == '-') {
      ...
      if (arg == "-o") {
        ...
        if (++argi >= argc) Error("missing file following: " + arg, true);
        if (!LoadTextFile(argv[argi], filenames)) Error("failed to read file: " + 
			std::string(argv[argi]), true);
      } else {
        for (size_t i = 0; i < params_.num_generators; ++i) {
