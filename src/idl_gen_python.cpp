...
class PythonGenerator : public BaseGenerator {
 public:
  ...
  void GetStartOfTableVerifier(const StructDef &struct_def, std::string *code_ptr) const {
    ...
    code += "def " + struct_def.name + "Verify(verifier, tablePos):\n";
    code += Indent + "result = True\n";
    code += Indent + "result = result and verifier.VerifyTableStart(tablePos)\n";
  }

  // Get the value of a table verification function end
  void GetEndOfTableVerifier(std::string *code_ptr) const {
    std::string &code = *code_ptr;
    code += Indent + "result = result and verifier.VerifyTableEnd(tablePos)\n";
  ...
  void GenVerifyCall(CodeWriter &code_, const FieldDef &field, const char *prefix)  const {
    ...
    if (IsScalar(field.value.type.base_type) || IsStruct(field.value.type)) {
      ...
      code_ +=
        "{{PRE}}{{INDENT1}}result = result and verifier.VerifyField(tablePos, "
    ...
    switch (field.value.type.base_type) {
      case BASE_TYPE_UNION: {
        ...
        code_ += "{{PRE}}{{INDENT1}}result = result and verifier.VerifyUnion(tablePos, {{TYPE_ID_OFFSET}}, "
          "{{OFFSET}}, {{ENUM_PACKAGE_PREFIX}}{{ENUM_NAME}}Verify, {{REQUIRED_FLAG}})  # field: {{NAME}}, type: [{{ENUM_NAME}}]";
        break;
      }
      case BASE_TYPE_STRUCT: {
        if (!field.value.type.struct_def->fixed) {
          code_ += "{{PRE}}{{INDENT1}}result = result and verifier.VerifyTable(tablePos, "
            "{{OFFSET}}, {{PACKAGE_REFERENCE_PREFIX}}{{TYPE}}Verify, {{REQUIRED_FLAG}})  # field: {{NAME}}, type: [{{TYPE}}]";
        }
        break;
      }
      case BASE_TYPE_STRING: {
        code_ += "{{PRE}}{{INDENT1}}result = result and verifier.VerifyString(tablePos, "
      ...
      case BASE_TYPE_VECTOR: {
        ...
        switch (field.value.type.element) {
          case BASE_TYPE_STRING: {
            code_ += "{{PRE}}{{INDENT1}}result = result and verifier.VerifyVectorOfStrings(tablePos, "
              "{{OFFSET}}, {{REQUIRED_FLAG}})  # field: {{NAME}}, type: [{{TYPE}}]";
            break;
          }
          case BASE_TYPE_STRUCT: {
            if (!field.value.type.struct_def->fixed) {
              code_ += "{{PRE}}{{INDENT1}}result = result and verifier.VerifyVectorOfTables(tablePos, "
                "{{OFFSET}}, {{PACKAGE_REFERENCE_PREFIX}}{{TYPE}}Verify, {{REQUIRED_FLAG}})  # field: {{NAME}}, type: [{{TYPE}}]";
            } else {
              code_.SetValue(
              ...
              code_ +=
                  "{{PRE}}{{INDENT1}}result = result and verifier.VerifyVectorOfData(tablePos, "
          ...
          default:
            ...
            if (!nfn.empty()) {
              ...
              code_ +="{{PRE}}{{INDENT1}}result = result and verifier.VerifyNestedBuffer(tablePos, "
  ...
  void GenUnionVerifier(const EnumDef &enum_def, std::string *code_ptr) const {
    ...
    code += "\n";
    code += "def " + enum_fn + "Verify(verifier, typeId, tablePos):";
  ...
  void GenUnionVerifierForStruct(const EnumDef &enum_def, const EnumVal &ev,
    ...
    if (! ev.union_type.struct_def->fixed) {
      code += GenIndents(2) + "return " + field_type + "Verify(verifier, tablePos)\n";
    } else {
      code += GenIndents(2) + "return  verifier.VerifyUnionData(tablePos, " +
