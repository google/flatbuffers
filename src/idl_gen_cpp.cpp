 ...
 private:
  ...
  void GenMemberAccessor(const FieldDef &field) {
    if (!field.deprecated &&  // Deprecated fields won't be accessible.
      ...
      code_.SetValue("FIELD_TYPE", full_type);
      code_.SetValue("PARENT_FIELD_NAME", "Parent::" + name);
      ...
      if (field.value.type.base_type == BASE_TYPE_VECTOR) {
        ...
        const bool gen_max_check = field.value.type.max_elements != 0;
        const auto element_type = GenTypeNative(field.value.type.VectorType(),
            true, field);
        code_.SetValue("MAX_ELEMENTS",
                       std::to_string(field.value.type.max_elements));
        ...
        if (gen_push) {
          ...
          if (gen_max_check) {
            code_ += "    if ({{PARENT_FIELD_NAME}}.size() >= {{MAX_ELEMENTS}})";
            code_ += "        return false;";
          }
        ...
        if (gen_emplace) {
          ...
          if (field.value.type.max_elements != 0) {
