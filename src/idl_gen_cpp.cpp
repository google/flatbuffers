...
class CppGenerator : public BaseGenerator {
 public:
  ...
  bool generate() {
    ...
    generateForwardDeclarations(&code);
    generateEnums(&code);
    generateStructs(&code);
    generateTables(&code);

    generateUnionVerifiers(&code);

    generateRootUtils(&code);
 ...
 private:
  void fileWarning(std::string *code_ptr) {
    *code_ptr +=
  ...
  void generateEnums(std::string *code_ptr) {
    for (auto it = parser_.enums_.vec.begin(); it != parser_.enums_.vec.end();
         ++it) {
      auto &enum_def = **it;
      if (!enum_def.generated) {
        CheckNameSpace(enum_def, code_ptr);
        GenEnum(parser_, enum_def, code_ptr);
      }
    }
  }

  // Generate code for all structs.
  void generateStructs(std::string *code_ptr) {
    for (auto it = parser_.structs_.vec.begin();
         it != parser_.structs_.vec.end(); ++it) {
      auto &struct_def = **it;
      if (struct_def.fixed && !struct_def.generated) {
        CheckNameSpace(struct_def, code_ptr);
        GenStruct(parser_, struct_def, code_ptr);
      }
    }
  }

  // Generate code for all tables.
  void generateTables(std::string *code_ptr) {
    for (auto it = parser_.structs_.vec.begin();
         it != parser_.structs_.vec.end(); ++it) {
      auto &struct_def = **it;
      if (!struct_def.fixed && !struct_def.generated) {
        CheckNameSpace(struct_def, code_ptr);
        GenTable(parser_, struct_def, code_ptr);
      }
    }
  }

  // Generate convenient global helper functions:
  void generateRootUtils(std::string *code_ptr) {
    if (parser_.root_struct_def_) {
      CheckNameSpace(*parser_.root_struct_def_, code_ptr);
      auto &name = parser_.root_struct_def_->name;
      std::string qualified_name =
          parser_.namespaces_.back()->GetFullyQualifiedName(name);
      std::string cpp_qualified_name = TranslateNameSpace(qualified_name);

      generateRootAccessors(name, cpp_qualified_name, code_ptr);
      generateRootVerifier(name, cpp_qualified_name, code_ptr);
      if (parser_.file_identifier_.length())
        generateIdentifierAccessors(name, code_ptr);
      if (parser_.file_extension_.length())
        generateExtensionAccessor(name, code_ptr);
      generateFinishRootBuffer(name, cpp_qualified_name, code_ptr);
    }
  }

  // Generate code for union verifiers.
  void generateUnionVerifiers(std::string *code_ptr) {
    for (auto it = parser_.enums_.vec.begin(); it != parser_.enums_.vec.end();
         ++it) {
      auto &enum_def = **it;
      if (enum_def.is_union && !enum_def.generated) {
        CheckNameSpace(enum_def, code_ptr);
        GenEnumPost(parser_, enum_def, code_ptr);
      }
    }
  }

  // The root datatype accessors:
  void generateRootAccessors(const std::string &name,
                             const std::string &cpp_qualified_name,
                             std::string *code_ptr) {
    std::string &code = *code_ptr;
  ...
  void generateIdentifierAccessors(const std::string &name,
                                   std::string *code_ptr) {
    std::string &code = *code_ptr;
  ...
  void generateExtensionAccessor(const std::string &name,
                                 std::string *code_ptr) {
    std::string &code = *code_ptr;
  ...
  // The root verifier:
  void generateRootVerifier(const std::string &name,
                            const std::string &cpp_qualified_name,
                            std::string *code_ptr) {
    std::string &code = *code_ptr;
  ...
  void generateFinishRootBuffer(const std::string &name,
                                const std::string &cpp_qualified_name,
                                std::string *code_ptr) {
    std::string &code = *code_ptr;
    ...
    if (parser_.file_identifier_.length()) code += ", " + name + "Identifier()";
    code += "); }\n\n";
  }
  ...
  void openGuard(std::string *code_ptr) {
    std::string include_guard = includeGuard();
    *code_ptr += "#ifndef " + include_guard + "\n";
    *code_ptr += "#define " + include_guard + "\n\n";
  }

  // Close the include guard.
  void closeGuard(std::string *code_ptr) {
    *code_ptr += "\n#endif  // " + includeGuard() + "\n";
  }

  void includeHeaders(std::string *code_ptr) {
    std::string &code = *code_ptr;
  ...
  void generateForwardDeclarations(std::string *code_ptr) {
    std::string &code = *code_ptr;
    for (auto it = parser_.structs_.vec.begin();
         it != parser_.structs_.vec.end(); ++it) {
      auto &struct_def = **it;
