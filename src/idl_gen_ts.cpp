...
class TsGenerator : public BaseGenerator {
 ...
 private:
    ...
    switch (field.value.type.base_type) {
      ...
      case BASE_TYPE_STRUCT: {
        if (!field.value.type.struct_def->fixed) {
          code_.SetValue("STRUCT_TYPE", AddImport(imports, *(field.value.type.struct_def), *(field.value.type.struct_def)).name);
          code_ += "{{PRE}}  result = result && verifier.verifyTable(tablePos, "
            "{{OFFSET}} /*{{NAME}}*/, {{STRUCT_TYPE}}Verify, {{REQUIRED_FLAG}});";
        }
        break;
      }
      ...
      case BASE_TYPE_VECTOR: {
        ...
        switch (field.value.type.element) {
          ...
          case BASE_TYPE_UNION: {
            code_.SetValue("ENUM_NAME", field.value.type.enum_def->name);
            code_.SetValue("TYPE_ID_VECTOR_OFFSET", NumToString(field.value.offset - sizeof(voffset_t)));
            code_.SetValue("STRUCT_TYPE",  AddImport(imports,  *(field.value.type.enum_def),  *(field.value.type.enum_def)).name);
            code_ += 
                "{{PRE}}  result = result && "
                "verifier.verifyVectorOfUnions(tablePos, "
                "{{TYPE_ID_VECTOR_OFFSET}}, {{OFFSET}} /*{{NAME}}*/, {{STRUCT_TYPE}}Verify, "
                "{{REQUIRED_FLAG}});";
            break;
          }
          default:
  ...
  void GetStartOfTableVerifier(const StructDef &struct_def, std::string *code_ptr) {
    ...
    code += "export function " + GetTypeName(struct_def) + "Verify";
...
//    cur_name_space_ = struct_def.defined_namespace;
  ...
  std::string GenSymbolExpression(const StructDef &struct_def,
    ...
    if (has_name_clash) {
      ...
      if (! struct_def.fixed) {
        symbols_expression += ", " + import_name + "Verify as "
          + name + "Verify";
      }
    } else {
      ...
      if (! struct_def.fixed) {
        symbols_expression += ", " + name + "Verify";
      }
    }
  ...
  std::string GenSymbolExpression(const EnumDef &enum_def,
    ...
    if (enum_def.is_union) {
      ...
      if (has_name_clash) {
        symbols_expression += ", " + import_name + "Verify as " + name + "Verify";
      } else {
        symbols_expression += ", " + name + "Verify";
      }
  ...
  std::string GenUnionVerify(const Type &union_type, import_set &imports) {
    if (union_type.enum_def) {
      ...
      auto ret = "\n\nexport function " + enum_def.name + "Verify" +
        "(verifier: flatbuffers.Verifier, typeId: number, tablePos: flatbuffers.UOffset) : boolean" +
        "{\n" +
        ...
        for (auto it = enum_def.Vals().begin(); it != enum_def.Vals().end(); ++it) {
          ...
          if (IsString(ev.union_type)) {
            ...
            if (! ev.union_type.struct_def->fixed) {
              ...
              ret += "      result = " + type + "Verify(verifier, tablePos);\n";
            } else {
