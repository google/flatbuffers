...
class TsGenerator : public BaseGenerator {
 public:
  // maps source to set of imported types
  using ImportedFileSet = std::unordered_set<std::string>;
  ...
  bool generate() {
    ...
    std::string import_code, enum_code, struct_code, code;
    generateEnums(&enum_code);
    generateStructs(&struct_code, importedFiles);
    generateImportDependencies(&import_code, importedFiles);
 ...
 private:
  ...
  void generateEnums(std::string *enum_code_ptr) {
    for (auto it = parser_.enums_.vec.begin(); it != parser_.enums_.vec.end();
         ++it) {
      auto &enum_def = **it;
      GenEnum(enum_def, enum_code_ptr);
    }
  }

  // Generate code for all structs.
  void generateStructs(std::string *decl_code_ptr, ImportedFileSet &importedFiles) {
    for (auto it = parser_.structs_.vec.begin();
         it != parser_.structs_.vec.end(); ++it) {
      auto &struct_def = **it;
      GenStruct(parser_, struct_def, decl_code_ptr, importedFiles);
    }
  }

  void GenNamespaces(std::string *code_ptr) {
  ...
  for (auto it = sorted_namespaces.begin();
    ...
    if (!parser_.opts.skip_flatbuffers_import && it->find('.') == std::string::npos) {
...
void GenStruct(const Parser &parser, StructDef &struct_def, std::string *code_ptr, ImportedFileSet &importedFiles) {
  if (struct_def.generated) return;
  std::string &code = *code_ptr;

  // Emit constructor
  std::string object_name = struct_def.name;
    ...
    // Emit an object field
      ...
      switch (field.value.type.base_type) {
        case BASE_TYPE_STRUCT: {
          ...
          importedFiles.insert(field.value.type.struct_def->file);
          break;
        }
        ...
        case BASE_TYPE_VECTOR: {
          ...
          if (vectortype.base_type == BASE_TYPE_STRUCT) {
            ...
            importedFiles.insert(vectortype.struct_def->file);
          } else if (vectortype.base_type == BASE_TYPE_STRING) {
            code += prefix + "):string\n";
