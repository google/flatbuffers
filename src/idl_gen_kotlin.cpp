...
/** possible improvements :
  B) correctness : implement flatbuffers features :
     ...
     8) original_order
     9) bit_flags
     10) improve the enum deserialization routine (use a transform function,
  skips, sparse array, list with binary search or a map)
     11) nested_flatbuffer
     12) key
     13) don't generate mutable methods if the flag is not set for flatc

  C) enhancements :
    1) allocation free toString() (requires recursion public
  __offset(byteBuffer, position) & cie)
    2) equals
    3) hashCode
    4) rewrite flatbuffer : copy parts from existing Struct/Table
    5) cache stuff that gets allocated (requires position into parent, lookup on
  get, clean on wrap)
    ...
    D) code beauty
      1) maybee we can downsize an int to a short through .toShort() instead of
  .and(0xFFFF).toShort()
      2) we should check the sign of constants before downsizing/upsizing it to
  avoid useless parenthesis like in (0).toByte()
*/
...
static const std::set<std::string> kotlinKeywords{
    "class",    "else",    "enum",    "fun",       "for",
    "if",       "in",      "is",      "import",    "internal",
    "override", "package", "private", "protected", "public",
    "return",   "val",     "var",     "when",      "while"};

class KotlinGenerator : public StronglyTypedGenerator {
 public:
  KotlinGenerator(const Parser &parser_, const std::string &path_,
                  const std::string &file_name_)
      : StronglyTypedGenerator(parser_, path_, file_name_, kotlinKeywords){};
 ...
 protected:
  ...
  bool generateStruct(const StructDef &struct_def) {
    ...
    code = code + struct_def.doc_comment + NL + "open class " +
           struct_def.name +
           "(byteBuffer : ByteBuffer = EMPTY_BYTEBUFFER) : Struct" +
           "(byteBuffer.order(ByteOrder.LITTLE_ENDIAN), if (byteBuffer === "
           "EMPTY_BYTEBUFFER) 0 else byteBuffer.getInt(byteBuffer.position()) "
           "+ byteBuffer.position()) {" +
           TAB;
    ...
    for (auto it = struct_def.fields.vec.begin();
      ...
      if ((field.value.type.base_type == BASE_TYPE_VECTOR &&
        ...
        generateBulkAccessors(field, code);
    }
  ...
  void structImports(const StructDef & /*struct_def*/, CodeWriter &code) {
    code = code + NL + "import java.nio.*;" + NL +
           "import com.google.flatbuffers.kotlin.*;" + NL + NL;
  }

  void generateStructAccessor(const StructDef & /*struct_def*/,
                              const FieldDef &field, CodeWriter &code) {
    code = code + field.doc_comment;
    if (IsScalar(field.value.type.base_type))
      getScalarFieldOfStruct(field, code);
    else
      switch (field.value.type.base_type) {
        case BASE_TYPE_STRUCT:
          getStructFieldOfStruct(field, code);
          return;
        case BASE_TYPE_STRING:
          getStringField(field, code);
          return;
        default:
          assert(0);
      }
  ...
  void getScalarFieldOfStruct(const FieldDef &field, CodeWriter &code) {
    ...
    code = code + NL + "var " + name(field) + " : " + userType(field) +
           " get() = " +
           upsizeToUserType(field.value.type,
                            wireGetter(field.value.type) + "(bb_pos + " +
                                NumToString(field.value.offset) + ")") +
           "; set(value) { " + wireSetter(field.value.type.base_type) +
  ...
  void getStructFieldOfStruct(const FieldDef &field, CodeWriter &code) {
    ...
    code = code + NL + "val " + name(field) + " : " + structName + " get() = " +
           name(field) + "(" + structName + "())" + NL + "fun " + name(field) +
           "(reuse : " + structName + ") : " + structName + " = " +
           "reuse.wrap(bb, bb_pos + " + NumToString(field.value.offset) + ")";
  }

  /** create struct */
  void generateCreateStruct(const StructDef &struct_def, CodeWriter &code) {
    code = code + NL;
    if (hasNestedStruct(struct_def))
      code += "inline ";  // only inline if there is a nested struct (or kotlin
                          // warns you)
    ...
    for (auto it = struct_def.fields.vec.begin();
      ...
      if (IsStruct(field.value.type))
        code = code + "crossinline " + name(field) +
               " : FlatBufferBuilder.()->Int";
      else
        code = code + name(field) + " : " + userTypeForCreation(field);
    }
    code = code + "):Int = with(this) {" + TAB;
    code = code + NL + "prep(" + NumToString(struct_def.minalign) + ", " +
           NumToString(struct_def.bytesize) + ")";
    for (auto it = struct_def.fields.vec.rbegin();
         it != struct_def.fields.vec.rend(); ++it) {
      auto &field = **it;
      if (field.padding)
        code = code + NL + "pad(" + NumToString(field.padding) + ")";
      if (IsStruct(field.value.type))
        code = code + NL + name(field) + "()";
      else {
        code = code + NL + "add" + wireScalarType(field.value.type.base_type) +
               "(" +
               downsizeToStorageValue(field.value.type, name(field), false) +
               ")";
      }
    }
    code = code + NL + "offset()" + BAT + NL + "}";
    ...
    code = code + "):FlatBufferBuilder.()->Int = {" +
           lowerFirst(struct_def.name) + "Raw(";
    for (auto it = struct_def.fields.vec.begin();
         it != struct_def.fields.vec.end(); ++it) {
      auto &field = **it;
      if (it != struct_def.fields.vec.begin()) code += ", ";
  ...
  bool generateTable(const StructDef &struct_def) {
    ...
    code = code + struct_def.doc_comment + NL + "open class " +
           struct_def.name +
           "(byteBuffer : ByteBuffer = EMPTY_BYTEBUFFER) : Table" +
           "(byteBuffer.order(ByteOrder.LITTLE_ENDIAN), if (byteBuffer === "
           "EMPTY_BYTEBUFFER) 0 else byteBuffer.getInt(byteBuffer.position()) "
           "+ byteBuffer.position()) {" +
           TAB;
  ...
  void generateTableAccessor(const StructDef & /*struct_def*/,
                             const FieldDef &field, CodeWriter &code) {
    code + field.doc_comment;
    if (IsScalar(field.value.type.base_type))
      getScalarFieldOfTable(field, code);
    else
      switch (field.value.type.base_type) {
        case BASE_TYPE_STRUCT:
          getStructFieldOfTable(field, code);
          return;
        case BASE_TYPE_STRING:
          getStringField(field, code);
          return;
        ...
        case BASE_TYPE_VECTOR: {
          if (field.value.type.element == BASE_TYPE_STRUCT)
            getMemberOfVectorOfStruct(field, code);
          else
            getMemberOfVectorOfNonStruct(field, code);
          return;
        }
        default:
          assert(0);
  ...
  void getScalarFieldOfTable(const FieldDef &field, CodeWriter &code) {
    code = code + NL + "val " + name(field) + " : " + userType(field) +
           ...
           // "null"; else code += defaultToUserType(field.value.type,
           // defaultValue(kotlinLang, field.value, false));
           +
           defaultToUserType(field.value.type, defaultValue(field.value, false))
           // !!!!
           + " else " +
           upsizeToUserType(field.value.type,
                            wireGetter(field.value.type) + "(o + bb_pos)") +
           "}";

    code =
        code + NL + "fun mutate" + Name(field) + "(value : " + userType(field) +
        ") :Boolean {" + "val o = __offset(" + NumToString(field.value.offset) +
        "); " + "return if (o == 0) false else {" +
        wireSetter(field.value.type.base_type) + "(o + bb_pos, " +
        downsizeToStorageValue(field.value.type, "value", true) + "); true}}";
  }
  ...
  void getStructFieldOfTable(const FieldDef &field, CodeWriter &code) {
    ...
    code = code + NL + "val " + name(field) + " : " + struct_def->name +
           "? get() {" + "val o = __offset(" + NumToString(field.value.offset) +
           "); " + "return if (o == 0) null else " + struct_def->name +
           "().wrap(bb, ";
    if (struct_def->fixed)
      code += "o + bb_pos)";
    else
      code += "__indirect(o + bb_pos))";
    code += "}";

    code = code + NL + "fun " + name(field) + "(reuse : " + struct_def->name +
           ") : " + struct_def->name + "? {" + "val o = __offset(" +
           NumToString(field.value.offset) + "); " +
  ...
  void getUnion(const FieldDef &field, CodeWriter &code) {
    ...
    code = code + NL + "val " + name(field) + " : Table? get() {" +
           "val o = __offset(" + NumToString(field.value.offset) + "); " +
           "return if (o == 0) null else __union(" +
           field.value.type.enum_def->name + ".toTable(" + name(field) +
           "Type), o)}";
  }

  void getArraySize(const FieldDef &field, CodeWriter &code) {
    code = code + NL + "val " + name(field) + "Size : Int " +
           "get() {val o = __offset(" + NumToString(field.value.offset) +
  ...
  void getMemberOfVectorOfStruct(const FieldDef &field, CodeWriter &code) {
    ...
    StructDef *struct_def = field.value.type.struct_def;
    code = code + NL + "fun " + name(field) + "(j :Int, reuse : " +
           struct_def->name + "? = null) : " + struct_def->name + " {" +
           "val o = __offset(" + NumToString(field.value.offset) + "); " +
           "return if (o == 0) " +
           "throw Exception(\"calling member $j of array " + name(field) +
           " which is either empty or unset\")" +
           " else {val x = __vector(o) + j" +
           multiplyBySizeOf(field.value.type.VectorType()) + "; " +
  ...
  void getMemberOfVectorOfNonStruct(const FieldDef &field, CodeWriter &code) {
    ...
    if (element == BASE_TYPE_STRING)
      ...
      code += userType(field);  // userScalarType(element);
    code = code + " {val o = __offset(" + NumToString(field.value.offset) +
           "); return if (o == 0) " +
           "throw Exception(\"calling member $j of array " + name(field) +
           " which is either empty or unset\")"
           // if (vector_type.base_type != BASE_TYPE_STRING) code +=
           // defaultToUserType(field.value.type, defaultValue(kotlinLang,
           // field.value, false) ); else code += "null";
           + " else " +
           upsizeToUserType(field.value.type,
                            wireGetter(vector_type) + "(__vector(o) + j" +
                                multiplyBySizeOf(vector_type) + ")") +
           "}";

    if (element == BASE_TYPE_STRING)
      generateBulkStringArrayAccessor(field, code);

    // mutation
    if (element != BASE_TYPE_STRING) {
      code = code + NL + "fun mutate" + Name(field) + "(j : Int, value : " +
             userType(field) + ") :Boolean {" + "val o = __offset(" +
             NumToString(field.value.offset) + "); " +
             "return if (o == 0) false else {" + wireSetter(element) +
             "(__vector(o) + j" + multiplyBySizeOf(vector_type) + ", " +
             downsizeToStorageValue(vector_type, "value", true) + "); true}}";
    }
  }

  void generateBulkStringArrayAccessor(const FieldDef &field,
                                       CodeWriter &code) {
    code = code + NL + "fun " + name(field) + "Buffer(j : Int) : ByteBuffer? " +
           " {val o = __offset(" + NumToString(field.value.offset) + "); " +
           "return if (o == 0) "
           // you really shouldn't expect elements from an array that is empty
           // or unset
           + "throw Exception(\"calling member $j of array " + name(field) +
           " which is either empty or unset\")" +
  ...
  void generateBulkAccessors(const FieldDef &field, CodeWriter &code) {
    ...
    code = code + " : ByteBuffer get() = __vector_as_bytebuffer(" +
           NumToString(field.value.offset) + ", " +
  ...
  void generateCreateTable(const StructDef &struct_def, CodeWriter &code) {
    ...
    // if there are only deprecated fields in this table, don't generate a
    // create method
    ...
    for (auto it = struct_def.fields.vec.begin();
      ...
      if (!IsScalar(field.value.type.base_type) && (!struct_def.fixed)) {
        if (IsStruct(field.value.type) && field.value.type.struct_def->fixed)
          code += " : FlatBufferBuilder.()->Int ";
        else
          code += " : Int ";
      } else
        code = code + " : " + userTypeForCreation(field);
    }
    ...
    for (auto it = struct_def.fields.vec.begin();
      ...
      if (!IsScalar(field.value.type.base_type) && (!struct_def.fixed)) {
        ...
        code += defaultToUserType(field.value.type,
                                  defaultValue(field.value, false));
    }
    // method body
    code = code + ") = with(this) {" + TAB + NL + "with(" +
           MakeCamel(struct_def.name, true) + " ) {" + TAB + NL +
    ...
    for (size_t size = struct_def.sortbysize ? sizeof(largest_scalar_t) : 1;
      ...
      for (auto it = struct_def.fields.vec.rbegin();
        ...
        if (!field.deprecated && (!struct_def.sortbysize ||
          ...
          code = code + NL + "add" +
                 wireTypeForCreation(
                     field) /*GenMethod(kotlinLang, field.value.type)*/ +
                 "(" + NumToString(offset) + ", ";
          if (!IsScalar(field.value.type.base_type)) {
            code += name(field);
            if (IsStruct(field.value.type) &&
          ...
          if (field.value.type.base_type == BASE_TYPE_BOOL)
            code += "!=0";  // hack to work with booleans... bad :(
          code += ")";
        }
      }
    }
  ...
  void generateTableBuilders(const StructDef &struct_def, CodeWriter &code) {
    for (auto it = struct_def.fields.vec.begin();
      ...
      if (IsScalar(field.value.type.element))
        buildArrayOfScalar(field, code, alignment, elem_size);
      else
        buildArrayOfTableOrString(field, code);
    }
    generateFinishBuffer(struct_def, code);
  }

  void buildArrayWithLambda(const FieldDef &field, CodeWriter &code,
                            int alignment, int elem_size) {
    code = code + NL + "inline fun FlatBufferBuilder." + name(field) +
           "(numElems : Int, action : FlatBufferBuilder.()->Unit) : Int "
           "{startArray(" +
           NumToString(elem_size) + ", numElems, " + NumToString(alignment) +
           "); action(); return endArray()}";
  }

  // TODO replace varargs with array
  void buildArrayOfScalar(const FieldDef &field, CodeWriter &code,
                          int alignment, int elem_size) {
    code = code + NL + "fun FlatBufferBuilder." + name(field) + "(data : " +
           userTypeForArray(field) + "): Int {startArray(" +
           NumToString(elem_size) + ", data.size, " + NumToString(alignment) +
           "); for (i in data.size - 1 downTo 0) add" +
           wireScalarType(field.value.type.element) + "(" +
           downsizeToStorageValue(field.value.type.VectorType(), "data[i]",
                                  false) +
           "); return endArray(); }";
    // TODO addToWire ???
  }
  ...
  void buildArrayOfTableOrString(const FieldDef &field, CodeWriter &code) {
    code = code + NL + "fun FlatBufferBuilder." + name(field) +
           "(offsets : IntArray) " + " : Int {startArray(4, offsets.size, ";
    // table or string
    if (field.value.type.struct_def != nullptr)
      code += NumToString(field.value.type.struct_def->minalign);
    else
      code += "4";  // look into this
    code +=
        "); for (i in offsets.size - 1 downTo 0) addOffset(offsets[i]); return "
        "endArray(); }";
  }
  ...
  void generateFinishBuffer(const StructDef &struct_def, CodeWriter &code) {
    ...
    if (parser.root_struct_def_ == &struct_def &&
        parser.file_identifier_.length())
      code = code + ", \"" + parser.file_identifier_ + "\"";
    code += ") }";
  }
  ...
  void getStringField(const FieldDef &field, CodeWriter &code) {
    code = code + NL + "val " + name(field) + " : String? get() {" +
           "val o = __offset(" + NumToString(field.value.offset) + "); " +
           "return if (o == 0) null else " + wireGetter(field.value.type) +
           "(o + bb_pos)" + "}";
  }

  void initializeTableOrStruct(const StructDef &struct_def, CodeWriter &code) {
    code = code + NL +
           "fun wrap(byteBuffer : ByteBuffer, position : Int = "
           "byteBuffer.getInt(byteBuffer.position()) + byteBuffer.position()) "
           ": " +
           struct_def.name + " = apply {bb = byteBuffer; bb_pos = position}";
  }

  void generateHasIdentifier(const StructDef & /*struct_def*/,
                             CodeWriter &code) {
    // Check if a buffer has the identifier.
    code = code + NL +
           "fun hasIdentifier(byteBuffer : ByteBuffer) :Boolean = "
           "Table.hasIdentifier(byteBuffer, \"" +
           parser.file_identifier_ + "\")";
  }

  /** for debugging only, this doesn't reuse accessors and creates new objects
   * like crazy */
  void toString(const StructDef &struct_def, CodeWriter &code) {
    code = code + NL + "override fun toString() : String = \"" +
           MakeCamel(struct_def.name, true) + "(";
    bool first = true;
    for (auto it = struct_def.fields.vec.begin();
         it != struct_def.fields.vec.end(); ++it) {
      auto &field = **it;
      ...
      if (field.value.type.base_type != BASE_TYPE_VECTOR)
        ...
        code = code + "${(0 until " + name(field) + "Size).map({" +
               name(field) +
  ...
  void enumImports(const EnumDef & /*enum_def*/, CodeWriter &code) {
    code = code + NL + "import java.nio.*;" + NL +
           "import com.google.flatbuffers.kotlin.*;" + NL;
  }

  void enumDeclaration(const EnumDef &enum_def, CodeWriter &code) {
    code = code + NL + "enum class " + enum_def.name + "(val value: " +
           userScalarType(enum_def.underlying_type.base_type) + ") {" + TAB;
  ...
  void deserializeEnum(const EnumDef &enum_def, CodeWriter &code) {
    if (isAnArithmeticProgression(enum_def))
      return deserializeEnumWithFunction(enum_def, code);
    if (enum_def.vals.vec.size() < 8)
      return deserializeEnumWithIfs(enum_def, code);
    deserializeEnumWithMap(enum_def, code);
  }

  void deserializeEnumMethodDeclaration(const EnumDef &enum_def,
                                        CodeWriter &code) {
    code = code + NL + "fun from(value : " +
           wireScalarType(enum_def.underlying_type.base_type) + ") : " +
           enum_def.name;
  }
  ...
  void deserializeEnumWithFunction(const EnumDef &enum_def, CodeWriter &code) {
    ...
    if (enum_def.vals.vec.size() == 1) {
      code = code + " = if  (value.toInt() == " +
             NumToString(enum_def.vals.vec[0]->value) + ") " +
             enum_def.vals.vec[0]->name +
             " else throw Exception(\"Bad enum value : $value\")";
      return;
    }
    int first = enum_def.vals.vec[0]->value;
    int r = enum_def.vals.vec[1]->value - first;  // this is > 0 as enum values
                                                  // must be specified in
                                                  // ascending order
    code += " = __enums[";
    /** trying to generate clean code that does res = (value - first) /r without
     * noise */
    ...
    if (first > 0)
      code + " - " + NumToString(first);
    else {
      if (first < 0) code += " + " + NumToString(-first);
    }
    if (parenthesisNeeded) code += ")";
    if (r != 1) code = code + " / " + NumToString(r);
    code += "]";
    /** we have to keep a reference to this array, to avoid the defensive array
     * copy hidden in the call to values() */
  ...
  void deserializeEnumWithMap(const EnumDef &enum_def, CodeWriter &code) {
    ...
    code =
        code +
        "= map[value.toInt()] ?: throw Exception(\"Bad enum value : $value\")";
  ...
  void unionDeclaration(const EnumDef &enum_def, CodeWriter &code) {
    code = code + NL + NL + "enum class " + enum_def.name + "(val value: " +
           userScalarType(enum_def.underlying_type.base_type) + ") {" + TAB;
  ...
  void unionToTable(const EnumDef &enum_def, CodeWriter &code) {
    code = code + NL + "fun toTable( value : " + enum_def.name +
           ") : Table = when (value) {" + TAB;
    for (auto it = enum_def.vals.vec.begin(); it != enum_def.vals.vec.end();
         ++it) {
      auto &ev = **it;
      if (it == enum_def.vals.vec.begin())
        code = code + NL + ev.name + " -> throw Exception(\"void union\")";
      else {
        if (ev.struct_def) {
          auto namespac = wrapInNameSpace(*ev.struct_def);
          if (namespac.length() > ev.name.length())
            code = code + NL + ev.name + " -> " + namespac + "()";
          else
            code = code + NL + ev.name + " -> " + namespace_name + "." +
                   ev.name + "()";
        } else
          code = code + NL + ev.name + " -> " + namespace_name + "." + ev.name +
                 "()";
      }
    }
    code = code + BAT + NL + "}";
  ...
  */
  ...
  std::string userType(const FieldDef &field) {
    ...
    if (type.struct_def != nullptr)
      return type.struct_def->name;  // struct or Table
    ...
    if (base_type == BASE_TYPE_UNION)
      ...
      return type.enum_def->name;  // enum or unionType (which is an enum)
  }

  /** create methods in this code generator use Int for all pointer type and
   * regular userTypes for scalar types*/
  std::string userTypeForArray(const FieldDef &field) {
    if (IsScalar(field.value.type.element)) {
      if (field.value.type.enum_def == nullptr)
        return userType(field) + "Array";
      else
        return "Array<" + userType(field) + ">";
    }
    return "Int";
  }
  ...
  std::string wireTypeForCreation(const FieldDef &field) {
    ...
    if (field.value.type.struct_def != nullptr &&
        field.value.type.struct_def->fixed && base_type != BASE_TYPE_VECTOR)
      return "Struct";
    else
      return "Offset";
  }
  ...
  std::string wireSetter(const BaseType &base_type) {
    if (wireScalarType(base_type) == "Byte" ||
        wireScalarType(base_type) == "Boolean")
      return "bb.put";
    else
      return "bb.put" + wireScalarType(base_type);
  }
  ...
  std::string wireGetter(const Type &type) {
    switch (type.base_type) {
      ...
      case BASE_TYPE_UNION:
        return "__union";
      case BASE_TYPE_VECTOR:
        return wireGetter(type.VectorType());
      default: {
  ...
  /** downsize a scalar userType to a scalar wireType
      ...
      2) with enums, you have to first transform the enum into a scalar userType
     before you turn it into a scalar wireType
      */
  std::string downsizeToStorageValue(const Type &type, const std::string value,
                                     const bool downsizeBool) {
    std::string val(value);
    if (type.enum_def != nullptr) val += ".value";
    switch (type.base_type) {
      case BASE_TYPE_BOOL:
        return downsizeBool ? "if (" + val + ") 1.toByte() else 0.toByte()"
                            : val;
      case BASE_TYPE_UINT:
        return val + ".and(0xFFFFFFFFL).toInt()";
      case BASE_TYPE_USHORT:
        return val +
               ".and(0xFFFF).toShort()";  // are "ands" really necessary here ?
      case BASE_TYPE_UCHAR:
        return val + ".and(0xFF).toByte()";
      case BASE_TYPE_VECTOR:
        return downsizeToStorageValue(type.VectorType(), value, downsizeBool);
      default:
        return val;
    }
  }
  ...
  std::string upsizeToUserType(const Type &type, const std::string value) {
    if (type.enum_def != nullptr && type.base_type != BASE_TYPE_UNION)
      return type.enum_def->name + ".from(" + value + ")";
    switch (type.base_type) {
      case BASE_TYPE_UINT:
      ...
      case BASE_TYPE_VECTOR:
        return upsizeToUserType(type.VectorType(), value);
      default:
        return value;
    }
  }
  ...
  std::string defaultToUserType(const Type &type, const std::string value) {
    // we have to catch enums before the switch because otherwise they follow
    // the path of their underlying base_type.
    if (type.enum_def != nullptr && type.base_type != BASE_TYPE_UNION)
      return type.enum_def->name + ".from(" + value + ")";
    switch (type.base_type) {
      case BASE_TYPE_UINT:
      ...
      case BASE_TYPE_UCHAR:
        return value + ".toInt().and(0xFF)";
      case BASE_TYPE_VECTOR:
        return defaultToUserType(type.VectorType(), value);
      default:
        return value;
    }
  }
  ...
  std::string defaultValue(const Value &value, bool for_buffer) {
    if (!for_buffer) {
      switch (value.type.base_type) {
        case BASE_TYPE_CHAR:
          return "(" + value.constant + ").toByte()";
        case BASE_TYPE_SHORT:
          return "(" + value.constant + ").toShort()";
        case BASE_TYPE_LONG:
        case BASE_TYPE_ULONG:
          return value.constant + "L";
        case BASE_TYPE_FLOAT:
          return value.constant + "f";
        case BASE_TYPE_VECTOR:
          switch (value.type.element) {
            case BASE_TYPE_CHAR:
              return "(" + value.constant + ").toByte()";
            case BASE_TYPE_SHORT:
              return "(" + value.constant + ").toShort()";
...
bool GenerateKotlin(const Parser &parser, const std::string &path,
  ...
  kotlin::KotlinGenerator *generator =
      new kotlin::KotlinGenerator(parser, path, file_name);
