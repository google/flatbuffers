...
class AsGenerator : public BaseGenerator {
 public:
  typedef std::map<std::string, ImportDefinition> import_set;

  AsGenerator(const Parser &parser, const std::string &path,
 ...
 private:
  ...
  void GenerateRootAccessor(StructDef &struct_def, std::string *code_ptr,
    ...
    if (!struct_def.fixed) {
      ...
      code += "  }\n";
      code +=
          "  return local.__init(bb.readInt32(bb.position()) + bb.position(), "
          "bb);\n";
      code += "};\n\n";
    }
  }
  ...
  std::string GenUnionGenericTypeAS(const EnumDef &union_enum) {
  ...
  std::string GenUnionTypeAS(const EnumDef &union_enum, import_set &imports) {
    std::string ret;
  ...
  // Generate a AS union type based on a union's enum
  std::string GenObjApiUnionTypeAS(import_set &imports, const IDLOptions &opts,
  ...
  std::string GenUnionConvFunc(const Type &union_type, import_set &imports) {
    if (union_type.enum_def) {
      ...
      const auto valid_union_type = GenUnionTypeAS(enum_def, imports);
  ...
  // type
  std::string GenUnionValAS(import_set &imports, const std::string &field_name,
  ...
  std::string GenStructMemberValueAS(const StructDef &struct_def,
    ...
    for (auto it = struct_def.fields.vec.begin();
      ...
      if (IsStruct(field.value.type)) {
        ret += GenStructMemberValueAS(*field.value.type.struct_def,
  ...
  void GenObjApi(const Parser &parser, StructDef &struct_def,
    ...
    if (struct_def.fixed) {
      ...
      pack_func_create_call +=
          GenStructMemberValueAS(struct_def, "this", ",\n    ", false) + "\n  ";
    }
        ...
        switch (field.value.type.base_type) {
          ...
          case BASE_TYPE_VECTOR: {
            ...
            switch (vectortype.base_type) {
              ...
              case BASE_TYPE_UNION: {
                field_type += GenObjApiUnionTypeAS(imports, parser.opts,
                                                   *(vectortype.enum_def));
                field_type += ")[]";
                field_val =
                    GenUnionValAS(imports, field_name, vectortype, true);
          ...
          case BASE_TYPE_UNION: {
            ...
            field_val = GenUnionValAS(imports, field_name, field.value.type);
            field_offset_decl =
      ...
      // Emit an object field
        ...
        switch (field.value.type.base_type) {
          ...
          case BASE_TYPE_VECTOR: {
            ...
            if (is_union) {
              const auto union_type =
                  GenUnionGenericTypeAS(*(field.value.type.enum_def));
          ...
          case BASE_TYPE_UNION: {
            ...
            const auto union_type = GenUnionGenericTypeAS(union_enum);
            code += "(obj:flatbuffers.Table):flatbuffers.Table|null{\n";
            code += offset_prefix +
        ...
        if (IsVector(field.value.type)) {
          ...
          if (!IsStruct(vector_type)) {
            ...
            if (type == "i32[]") {
              ...
              std::string type_old = "i32[]|Uint8Array";
              std::string type_new = "i32[]|" + array_type + "Array";
...
bool GenerateAS(const Parser &parser, const std::string &path,
                const std::string &file_name) {
  as::AsGenerator generator(parser, path, file_name);
  return generator.generate();
}

std::string ASMakeRule(const Parser &parser, const std::string &path,
                       const std::string &file_name) {
