...
void BinaryAnnotator::BuildTable(uint64_t offset, const BinarySectionType type,
  ...
  // This is the size of this table.
  ...
  for (auto vtable_field : vtable.fields) {
    ...
    if (field == nullptr) {
      ...
      // auto next_field = vtable.fields.find(field_id + 1);

      // // Calcualte the length of this unknown field.
      // const uint16_t unknown_field_length =
      //     (next_field != vtable.fields.end()
      //          ? next_field->second.offset_from_table
      //          : table_end_offset) -
      //     field_offset;

      const uint16_t unknown_field_length = table_end_offset - field_offset;
    ...
    if (offset < section_start_offset) {
      ...
      if (IsNonZeroRegion(offset - 1, pad_bytes, binary_)) {
        ...
        sections_to_insert.emplace_back(MakeBinarySection(
            "no known references", BinarySectionType::Unknown, std::move(regions)));
      } else {
