...
class GoGenerator : public BaseGenerator {
 ...
 private:
  ...
  void GetStartOfTableVerifier(const StructDef &struct_def, std::string *code_ptr) {
    ...
    code += "func " + struct_def.name + "Verify";
      ...
      if (tracked_imported_namespaces_.size() > 0) {
        ...
        for (auto it = tracked_imported_namespaces_.begin();
          ...
          if ((*it)->defined_namespace->components.empty()) {
            code += "\t" + (*it)->name + " \"" + NameImportPath((*it)->name) + "\"\n";
          } else {
  ...
  // Create the full path for the imported name (format: A/B/C).
  // This function  assumes there is no namespace but includes module name
  // in path
  std::string NameImportPath(const std::string &name) const {
