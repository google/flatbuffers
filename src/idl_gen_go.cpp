...
static void GenStructAccessor(const StructDef &struct_def,
  ...
  if (IsScalar(field.value.type.base_type)) {
    if (struct_def.fixed) {
      lang::GetScalarFieldOfStruct(struct_def, field, code_ptr);
    } else {
      lang::GetScalarFieldOfTable(struct_def, field, code_ptr);
    }
  } else {
    switch (field.value.type.base_type) {
      case BASE_TYPE_STRUCT:
        if (struct_def.fixed) {
          lang::GetStructFieldOfStruct(struct_def, field, code_ptr);
        } else {
          lang::GetStructFieldOfTable(struct_def, field, code_ptr);
        }
        break;
      case BASE_TYPE_STRING:
        lang::GetStringField(struct_def, field, code_ptr);
        break;
      case BASE_TYPE_VECTOR: {
        auto vectortype = field.value.type.VectorType();
        if (vectortype.base_type == BASE_TYPE_STRUCT) {
          lang::GetMemberOfVectorOfStruct(struct_def, field, code_ptr);
        } else {
          lang::GetMemberOfVectorOfNonStruct(struct_def, field, code_ptr);
        }
        break;
      }
      case BASE_TYPE_UNION:
        lang::GetUnionField(struct_def, field, code_ptr);
        break;
      default:
        assert(0);
    }
  ...
  if (field.value.type.base_type == BASE_TYPE_VECTOR) {
    lang::GetVectorLen(struct_def, field, code_ptr);
  }
}
...
static void GenTableBuilders(const StructDef &struct_def,
  ...
  for (auto it = struct_def.fields.vec.begin();
    ...
    if (field.value.type.base_type == BASE_TYPE_VECTOR) {
      lang::BuildVectorOfTable(struct_def, field, code_ptr);
    }
  }
