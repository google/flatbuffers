...
class FlatBufferBuilder {
    ...
    /*
    inline public fun arrayOf(elem_size: Int, num_elems: Int, alignment: Int, action:FlatBufferBuilder.()->Unit) : Int {
        startArray(elem_size, num_elems, alignment)
        action()
        return endArray()
    }


    class IntsOffset(val offset:Int)
    public fun intsOf(vararg data:Int) :IntsOffset {
        startArray(4, data.size, 4)
            for (i in data.size - 1 downTo 0) addInt(data[i])
            return IntsOffset(endArray())
    }
    */
    // arrayOfOffsets :Array<Offset> -> addOffset for structs
    // arrayOfValues Array<Type> -> addType       for scalars
    ...
    fun finish(root_table: Int, file_identifier: String) {
        ...
        for (i in FILE_IDENTIFIER_LENGTH - 1 downTo 0) addByte(file_identifier[i].toByte())
        finish(root_table)
    }
