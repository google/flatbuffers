...
the `flatc` flag `--gen-object-api` is used. For example, this schema:
    ...
    table Thing {
      value:short;
      list:[Nested];
      nested:[Nested] (max_size: 40); 
      str:[string] (max_size: 20); 
    }
...
Guarded API class:
    ...
    class GuardedThing : private ThingT {
     public:
      ...
      void clear();  // Clears all vectors and strings.
      ...
      Nested* add_list();
      bool pop_list();
      bool swap_list(std::vector<Nested>& value);
      const std::vector<Nested>& list() const &;
      size_t list_size() const;
      void clear_list();

      static constexpr size_t NESTED_MAX_SIZE() { return 40; }
      bool push_nested(const Nested& value);
      bool emplace_nested(Nested&& value);
      Nested* add_nested();
      bool pop_nested();
      bool swap_nested(std::vector<Nested>& value);
      const std::vector<Nested>& nested() const &;
      size_t nested_size() const;
      void clear_nested();

      static constexpr size_t STR_MAX_SIZE() { return 20; }
      bool set_str(const std::string& value);
      bool set_str(std::string&& value);
      bool swap_str(std::string& value);
