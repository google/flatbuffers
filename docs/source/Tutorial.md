...
<form>
  ...
  <input type="radio" name="language" value="julia">Julia</input>
</form>
\endhtmlonly
...
<div class="language-julia">
~~~{julia}
  # types are not built up gradually in Julia,
  # instead they are written to the buffer all at once
  buf = IOBuffer()
~~~
</div>
...
<div class="language-julia">
~~~{.jl}
  # we don't need to incrementally build things up here, just make the monster
  orc = Monster(;
    name="Orc",
...
<div class="language-julia">
~~~{.jl}
  path = Vec3[Vec3(1.0, 2.0, 3.0), Vec3(4.0, 5.0, 6.0)]
~~~
</div>
...
<div class="language-julia">
  ~~~{.jl}
    # this section is unnecessary in Julia
  ~~~
</div>
...
<div class="language-julia">
~~~{.jl}
  # where x is either an IO or an AbstractVector{UInt8} containing the data
  monster = Monster(x)
~~~
</div>
...
<div class="language-julia">
~~~{.jl}
  third_item = monster.inventory[3]
~~~
</div>
...
<div class="language-julia">
~~~{.jl}
  # no tables are needed in Julia
  second_weapon_name = monster.weapons[2].name
  second_weapon_damage = monster.weapons[2].damage
~~~
</div>
...
<div class="language-julia">
~~~{.jl}
  <API for mutating FlatBuffers is not yet available in Julia.>
~~~
</div>
