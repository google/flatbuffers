...
for scalar types, or `NULL` for other types. Some languages support
setting a scalar's default to `null`. This makes the scalar optional.
...
-    Default valued fields will return the default value.
-    Optional valued fields will return some form of `null` depending on the
     local language. (In a sense, `null` is the default value).
-    Required fields will cause an error. Flatbuffer verifiers would
