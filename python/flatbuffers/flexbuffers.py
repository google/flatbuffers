...
def _Unpack(fmt: str, buf: bytes):
    return struct.unpack("<%s" % fmt[len(buf)], buf)[0]


def _UnpackVector(fmt: str, buf: bytes, length: int):
    byte_width = len(buf) // length
    return struct.unpack("<%d%s" % (length, fmt[byte_width]), buf)


def _Pack(fmt: str, value: Any, byte_width: int):
    return struct.pack("<%s" % fmt[byte_width], value)


def _PackVector(fmt: str, values: List[Any], byte_width: int):
