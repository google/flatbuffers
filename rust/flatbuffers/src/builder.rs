...
mod tests {
    ...
    /// uninitialized memory bugs. See https://github.com/google/flatbuffers/issues/8894
    ...
    fn test_vtable_zeroed_with_garbage_allocator() {
        ...
        let vtable_offset =
            i32::from_le_bytes([data[table_pos], data[table_pos + 1], data[table_pos + 2], data[table_pos + 3]]);
        ...
        assert_eq!(vtable_size, 16, "VTable should be 16 bytes (4 header + 6*2 fields)");
        ...
        for i in [4_usize, 6, 8, 10, 12] {
            let field_offset =
                u16::from_le_bytes([data[vtable_pos + i], data[vtable_pos + i + 1]]);
            assert_eq!(
                field_offset, 0,
        ...
        let field5_offset =
            u16::from_le_bytes([data[vtable_pos + 14], data[vtable_pos + 14 + 1]]);
